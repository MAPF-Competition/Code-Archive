...
public: 
    MatchingPriorityQueue(int num_agents, int num_tasks): 
        ...
        pqs(num_agents),
        edges(num_agents, std::vector<std::tuple<COST_TYPE, int, int> >(num_tasks)),
    ...
    std::vector<std::priority_queue<std::tuple<COST_TYPE, int, int> > > pqs;
    std::vector<std::vector<std::tuple<COST_TYPE, int, int> > > edges;
    ...
    std::tuple<COST_TYPE,int,int> _pop() {
        ...
        while (true) {
            ...
            gpq.pop();
            if (!matched_agents[i_agent]) { // TODO: we don't need check this line
