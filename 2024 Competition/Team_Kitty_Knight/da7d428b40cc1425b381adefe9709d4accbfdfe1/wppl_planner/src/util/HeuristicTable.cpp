...
HeuristicTable::HeuristicTable(SharedEnvironment * _env, const std::shared_ptr<std::vector<float> > & map_weights, bool consider_rotation):
    ...
    char * temp=getenv("SHARE_HEURISTICS");
    if (temp!=NULL && std::string(temp)!="") {
        ONLYDEV(std::cout<<"share heuristics: no computation"<<std::endl;)
        share_heuristics=true;
    }

    // bad code: strip .map
    if (env.map_name.find(".map")==-1) {
        std::cerr<<"error: map name should not contain .map: "<<env.map_name<<std::endl;
        exit(-1);
    }

    shm_name_base = env.map_name.substr(0,env.map_name.find(".map"));
    shm_name_heuristics = shm_name_base+"_heuristics";
    shm_name_empty_locs = shm_name_base+"_empty_locs";
    shm_name_loc_idxs = shm_name_base+"_loc_idxs";
    shm_size_empty_locs = loc_size*sizeof(int);
    shm_size_loc_idxs = env.map.size()*sizeof(int);
    shm_size_heuristics = state_size*loc_size*sizeof(float);

    // NOTE: only one process need to create this
    if (!share_heuristics) {
        if (!create_shared_memory()) {
            std::cerr<<"error: failed to create shared memories"<<std::endl;
            exit(-1);
        }

        std::cout<<"successfully created shared memories"<<std::endl;
    } else {
        if (!load_shared_memory()) {
            std::cerr<<"error: failed to load shared memories"<<std::endl;
            exit(-1);
        }

        ONLYDEV(std::cout<<"successfully loaded shared memories"<<std::endl;)
    }

    // TODO(rivers): still has bug
    // if (!load_shared_memory()) {
    //     std::cerr<<"error: failed to load shared memories"<<std::endl;
    //     exit(-1);
    // }

    // heuristics = new float[state_size*loc_size];
    // std::fill(heuristics,heuristics+state_size*loc_size,MAX_HEURISTIC);
};

void * HeuristicTable::_create_shared_memory(string & shm_name, size_t shm_size) {
...
bool HeuristicTable::create_shared_memory() {
    void * _empty_locs, * _loc_idxs, * _heuristics;
    
    _empty_locs=_create_shared_memory(shm_name_empty_locs, shm_size_empty_locs);
    _loc_idxs=_create_shared_memory(shm_name_loc_idxs, shm_size_loc_idxs);
    _heuristics=_create_shared_memory(shm_name_heuristics, shm_size_heuristics);

    if (_empty_locs==MAP_FAILED || _loc_idxs==MAP_FAILED || _heuristics==MAP_FAILED) {
        return false;
    }

    std::cout<<"created shared memories: "<<shm_name_empty_locs<<","<<shm_name_loc_idxs<<","<<shm_name_heuristics<<std::endl;

    empty_locs = static_cast<int*>(_empty_locs);
    loc_idxs = static_cast<int*>(_loc_idxs);
    heuristics = static_cast<float*>(_heuristics);
...
bool HeuristicTable::_free_shared_memory(void * ptr, string & shm_name, size_t shm_size) {
    // Cleanup
    munmap(ptr, shm_size);
    if (!share_heuristics){
        shm_unlink(shm_name.c_str()); // Optional: Remove the shared memory object
    }
    return true;
}

bool HeuristicTable::free_shared_memory() {
    // Cleanup
    _free_shared_memory(empty_locs, shm_name_empty_locs, shm_size_empty_locs);
    _free_shared_memory(loc_idxs, shm_name_loc_idxs, shm_size_loc_idxs);
    _free_shared_memory(heuristics, shm_name_heuristics, shm_size_heuristics);

    ONLYDEV(std::cout<<"freed shared memories"<<std::endl;)
    return true;
}

void * HeuristicTable::_load_shared_memory(string & shm_name, size_t shm_size) {
    // Create shared memory
    int fd = shm_open(shm_name.c_str(),  O_RDWR , 0666);
    if (fd == -1) {
        std::cout<<shm_name<<std::endl;
        perror((std::string("Error creating shared memory ")+shm_name).c_str());
        return MAP_FAILED;
    }
...
bool HeuristicTable::load_shared_memory() {
    void * _empty_locs, * _loc_idxs, * _heuristics;
    
    _empty_locs=_load_shared_memory(shm_name_empty_locs, shm_size_empty_locs);
    _loc_idxs=_load_shared_memory(shm_name_loc_idxs, shm_size_loc_idxs);
    _heuristics=_load_shared_memory(shm_name_heuristics, shm_size_heuristics);
...
float HeuristicTable::get(int loc1, int loc2) {
    ...
    for (size_t orient=0;orient<4;++orient) {
        size_t idx=(loc_idx1*loc_size+loc_idx2)*n_orientations+orient;
        if (heuristics[idx]<cost) {
            cost=heuristics[idx];
        }
    }
...
float HeuristicTable::get(int loc1, int orient1, int loc2) {
    ...
    size_t idx=(loc_idx1*loc_size+loc_idx2)*n_orientations+orient1;
...
// weights is an array of [loc_size*n_orientations]
void HeuristicTable::compute_weighted_heuristics_single_process(){
    // DEV_DEBUG("[start] Compute heuristics.");
    ONLYDEV(g_timer.record_p("heu/compute_start");)
...
#ifdef MAP_OPT
        compute_weighted_heuristics_single_process();
#else
        compute_weighted_heuristics();
#endif
        // ONLYDEV(save(fpath));
