...
{
public:
	list<EECBS_PREFIX_Constraint> constraints; // new constraints
	...
	list<shared_ptr<EECBS_PREFIX_Conflict> > conflicts;
	list<shared_ptr<EECBS_PREFIX_Conflict> > unknownConf;

	// The chosen conflict
	shared_ptr<EECBS_PREFIX_Conflict> conflict;
...
public:
	...
	list< pair< int, EECBS_PREFIX_Path> > paths; // new paths
...
struct ConstraintsHasher // Hash a CT node by constraints on one agent
{
	...
	struct EqNode
	{
		bool operator() (const ConstraintsHasher& c1, const ConstraintsHasher& c2) const
		{
			...
			std::set<EECBS_PREFIX_Constraint> cons1, cons2;
			auto curr = c1.n;
