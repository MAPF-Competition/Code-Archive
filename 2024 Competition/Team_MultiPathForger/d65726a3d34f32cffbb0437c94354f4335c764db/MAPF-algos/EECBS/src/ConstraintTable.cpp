...
int ConstraintTable::getMaxTimestep() const // everything is static after the max timestep
{
    ...
    if (length_max < EECBS_MAX_TIMESTEP)
...
void ConstraintTable::insert2CT(size_t loc, int t_min, int t_max)
{
    ...
    if (t_max < EECBS_MAX_TIMESTEP && t_max > ct_max_timestep)
    {
        ct_max_timestep = t_max;
    }
    else if (t_max == EECBS_MAX_TIMESTEP && t_min > ct_max_timestep)
    {
...
void ConstraintTable::insert2CT(const list<EECBS_PREFIX_Constraint>& constraints, int agent)
{
    ...
    switch (type)
    {
        case constraint_type::LEQLENGTH:
            ...
            if (agent == a) // this agent has to reach its goal at or before timestep t.
                ...
                insert2CT(x, t, EECBS_MAX_TIMESTEP);
            break;
...
void ConstraintTable::insert2CT(const EECBS_PREFIX_Path& path)
{
    ...
    insert2CT(path.back().location, (int) path.size() - 1, EECBS_MAX_TIMESTEP);
}
...
void ConstraintTable::insert2CAT(int agent, const vector<EECBS_PREFIX_Path*>& paths)
{
...
void ConstraintTable::insert2CAT(const EECBS_PREFIX_Path& path)
{
    if (cat.empty())
    {
        cat.resize(map_size);
        cat_goals.resize(map_size, EECBS_MAX_TIMESTEP);
    }
    if (cat_goals[path.back().location] != EECBS_MAX_TIMESTEP) {
        std::cout << path.front().location << " "<< path.back().location << "    " << EECBS_MAX_TIMESTEP << "  " << cat_goals[path.back().location] << std::endl;
        for (int i = 0; i < path.size(); i++) {
            std::cout << "(" << path[i].location / 32 << ","  << path[i].location % 32 << ")" << std::endl; 
        }
    }
    assert(cat_goals[path.back().location] == EECBS_MAX_TIMESTEP);
