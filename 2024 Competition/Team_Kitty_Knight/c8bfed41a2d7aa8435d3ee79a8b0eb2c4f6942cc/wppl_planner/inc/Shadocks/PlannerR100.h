...
class PlannerR100 : public Planner
{
public:
    PlannerR100();
...
public:
    ...
    struct Node {
        ...
        Node* parent;
        std::bitset<100> conflicts; // Bitset gives 10% more A* searches in 1 second
        bool closed = false;
        Node(){};
...
private:
    // Paths
    std::vector<int> plan_path(int agent, const Position &start, double timeleft);
    std::vector<int> plan_dummy_path(int agent, const Position &start, double timeleft);
    bool update_paths();
    ...
    std::pair<int, std::bitset<100>> add_conflicts(Node * curr, const std::vector<int> & conflicts) const;
    ...
    void shuffle(std::queue<int> & queue);
