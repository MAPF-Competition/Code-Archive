...
struct Parameters {
    ...
    bool halt = false;
    bool distance_map = false; // Build a distance map
...
public:
    ...
    /**
     * @brief The AstarNode struct with conflicts
     * Node for the A* search algorithm
     */
    struct AstarNC {
        int location;
        ...
        // tsl::hopscotch_set<int> conflicts;
        std::bitset<200> conflicts; // Bitset gives 10% more A* searches in 1 second
        double conflict_cost = 0;
        bool closed = false;
        AstarNC(){};
        AstarNC(int _location,int _direction, int _time, float _g, int _h, AstarNC* _parent, int _g2 = -1)
            : location(_location)
            , direction(_direction)
            , time(_time), f(_g+_h)
            , g(_g)
...
protected:
    ...
    void make_distance_map(int location);
    ...
    std::list<int> location_neighbors(int loc) const;
    void make_distance_map_json() const;
    void write_distance_map_json() const;
...
protected:
    ...
    std::vector<int> m_distance_map; // m_distance_map[loc] = distance from loc to the task of agent a @todo Certainly to remove, this was slow
    std::vector<std::list<int>> m_passing; // m_passing[loc] = agents passing by a location
    std::unordered_map<pair<int,int>,std::vector<int>> m_passing_hash;
    std::unordered_map<int,std::vector<int>> m_ending_hash;
