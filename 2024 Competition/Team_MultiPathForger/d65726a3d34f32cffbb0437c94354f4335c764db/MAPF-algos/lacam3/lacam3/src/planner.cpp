...
bool LACAM_PREFIX_Planner::FLG_SWAP = true;
bool LACAM_PREFIX_Planner::FLG_STAR = true;
bool LACAM_PREFIX_Planner::FLG_MULTI_THREAD = true;
int LACAM_PREFIX_Planner::SCATTER_MARGIN = 10;
int LACAM_PREFIX_Planner::PIBT_NUM = 10;
bool LACAM_PREFIX_Planner::FLG_REFINER = true;
int LACAM_PREFIX_Planner::REFINER_NUM = 4;
bool LACAM_PREFIX_Planner::FLG_SCATTER = true;
float LACAM_PREFIX_Planner::RANDOM_INSERT_PROB1 = 0.1;
float LACAM_PREFIX_Planner::RANDOM_INSERT_PROB2 = 0.01;
bool LACAM_PREFIX_Planner::FLG_RANDOM_INSERT_INIT_NODE = false;
float LACAM_PREFIX_Planner::RECURSIVE_RATE = 0.2;
double LACAM_PREFIX_Planner::RECURSIVE_TIME_LIMIT = 1000;

std::string LACAM_PREFIX_Planner::MSG;
int LACAM_PREFIX_Planner::CHECKPOINTS_DURATION = 5000;
constexpr int CHECKPOINTS_NIL = -1;

constexpr auto TIME_ZERO = std::chrono::seconds(0);

LACAM_PREFIX_Planner::LACAM_PREFIX_Planner(const LACAM_PREFIX_Instance *_ins, int _verbose, const Deadline *_deadline,
                 int _seed, int _depth, LACAM_PREFIX_DistTable *_D)
    : ins(_ins),
      deadline(_deadline),
      seed(_seed),
      MT(std::mt19937(seed)),
      ...
      N(ins->N),
      V_size(ins->G->size()),
      D((_D == nullptr) ? new LACAM_PREFIX_DistTable(ins) : _D),
...
LACAM_PREFIX_Planner::~LACAM_PREFIX_Planner()
{
...
LACAM_PREFIX_Solution LACAM_PREFIX_Planner::solve()
{
  ...
  while (!OPEN.empty() && !is_expired(deadline)) {
    ...
    refiner_pool.remove_if([&](auto &proc) {
      ...
      refiner_pool.emplace_back(std::async(std::launch::async,
                                           &LACAM_PREFIX_Planner::get_refined_plan, this,
                                           backtrack(H_goal)));
      return true;
    });
    ...
    auto Q_to = LACAM_PREFIX_Config(N, nullptr);
...
LACAM_PREFIX_HNode *LACAM_PREFIX_Planner::create_highlevel_node(const LACAM_PREFIX_Config &Q, LACAM_PREFIX_HNode *parent)
{
  auto g_val =
      (parent == nullptr) ? 0 : parent->g + get_edge_cost(parent->C, Q);
  auto h_val = heuristic->get(Q);
  auto H_new = new LACAM_PREFIX_HNode(Q, D, parent, g_val, h_val);
  EXPLORED[Q] = H_new;
  return H_new;
}

void LACAM_PREFIX_Planner::apply_new_solution(const LACAM_PREFIX_Solution &plan)
{
  if (plan.empty()) return;
  info(3, verbose, deadline, "incorporate new solution");

  // forcibly insert configuration
  LACAM_PREFIX_HNode *H_from = EXPLORED[plan[0]];
  LACAM_PREFIX_HNode *H_to = nullptr;
  for (auto t = 1; t < plan.size(); ++t) {
    ...
    if (iter != EXPLORED.end()) {
      ...
      H_to = new LACAM_PREFIX_HNode(Q, D, H_from, g_val, heuristic->get(Q));
      EXPLORED[Q] = H_to;
...
LACAM_PREFIX_Solution LACAM_PREFIX_Planner::backtrack(LACAM_PREFIX_HNode *H)
{
  std::vector<LACAM_PREFIX_Config> plan;
  auto _H = H;
  while (_H != nullptr) {
    plan.push_back(_H->C);
    _H = _H->parent;
...
bool LACAM_PREFIX_Planner::set_new_config(LACAM_PREFIX_HNode *H, LACAM_PREFIX_LNode *L, LACAM_PREFIX_Config &Q_to)
{
  // worker-id, time -> configuration
  auto Q_cands = std::vector<LACAM_PREFIX_Config>(PIBT_NUM, LACAM_PREFIX_Config(N, nullptr));
...
void LACAM_PREFIX_Planner::rewrite(LACAM_PREFIX_HNode *H_from, LACAM_PREFIX_HNode *H_to)
{
  // update neighbors
  H_from->neighbor.insert(H_to);

  // Dijkstra
  std::queue<LACAM_PREFIX_HNode *> Q({H_from});  // queue is sufficient
  while (!Q.empty()) {
    auto n_from = Q.front();
    Q.pop();
...
int LACAM_PREFIX_Planner::get_edge_cost(const LACAM_PREFIX_Config &C1, const LACAM_PREFIX_Config &C2)
{
  auto cost = 0;
  for (uint i = 0; i < N; ++i) {
...
void LACAM_PREFIX_Planner::set_scatter()
{
...
void LACAM_PREFIX_Planner::set_pibt()
{
  for (auto k = 0; k < PIBT_NUM; ++k) {
    pibts.emplace_back(new LACAM_PREFIX_PIBT(ins, D, k + seed, FLG_SWAP, scatter));
  }
}

void LACAM_PREFIX_Planner::set_refiner()
{
  ...
  for (auto k = 0; k < REFINER_NUM; ++k) {
    ...
    refiner_pool.emplace_back(
        std::async(std::launch::async, &LACAM_PREFIX_Planner::get_refined_plan, this, plan));
  }
}

LACAM_PREFIX_Solution LACAM_PREFIX_Planner::get_refined_plan(const LACAM_PREFIX_Solution &plan)
{
  ...
  if (depth < 1 && plan.size() > 3 &&
    ...
    // recursive LaCAM
    auto ins_tmp =
        LACAM_PREFIX_Instance(ins->G, plan[get_random_int(MT_internal, 1, plan.size() - 2)],
                 ins->goals, N);
    ...
    auto planner_tmp =
        LACAM_PREFIX_Planner(&ins_tmp, 0, &deadline_tmp, seed_refiner, depth + 1, D);
    ...
    return LACAM_PREFIX_Solution();
  }
}

void LACAM_PREFIX_Planner::update_checkpoints()
{
...
void LACAM_PREFIX_Planner::logging()
{
  if (depth > 0) return;
  MSG += "checkpoints=";
  ...
  MSG += "\nnum_high_level_node=" + std::to_string(LACAM_PREFIX_HNode::COUNT);
  MSG += "\nnum_low_level_node=" + std::to_string(LACAM_PREFIX_LNode::COUNT);
