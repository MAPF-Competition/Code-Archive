...
bool KivaGrid::load_env_map(SharedEnvironment* env)
{
    // types = "KIVA";
    rows = env->rows;
    cols = env->cols;
    ...
    types.resize(env->map.size());
    weights.resize(env->map.size());

    for (int i = 0; i < env->map.size(); i++)
    {
        types[i] = env->map[i] ? "Obstacle" : "Travel";
        weights[i].resize(5, WEIGHT_MAX);
        if (!env->map[1]) weights[i][4] = 1;
    }
...
void KivaGrid::computeGoalHeuristics(int goal)
{
    auto iter = heuristics.find(goal);
    if (iter == heuristics.end())
    {
        heuristics[goal] = compute_heuristics(goal);
        auto result = heuristics[goal];
    }
}
