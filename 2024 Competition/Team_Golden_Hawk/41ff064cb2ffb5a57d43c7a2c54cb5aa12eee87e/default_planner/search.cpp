...
/**
 * @brief astar search minimized the opposide traffic flow with existing traffic flow
 * @param env the shared environment(input)
 * @param flow the current traffic flow(input)
 * @param ht the heuristic table(input)
 * @param traj the trajectory to store the path(input&output)
 * @param mem the memory pool(input)
 * @param start the start location(input)
 * @param goal the goal location(input)
 * @param ns the neighbors(input)
 */
