...
private:
    const BasicGraph& G;
    // Constraint Table (CT)
    boost::unordered_map<size_t, list<pair<int, int> > > ct; // location/edge -> time range
    // Conflict Avoidance Table (CAT)
    vector<vector<bool> > cat; //  (timestep, location) ->  have conflicts or not
    // Safe Interval Table (SIT)
    boost::unordered_map<size_t, list<Interval > > sit; // location/edge -> [t_min, t_max), num_of_collisions
