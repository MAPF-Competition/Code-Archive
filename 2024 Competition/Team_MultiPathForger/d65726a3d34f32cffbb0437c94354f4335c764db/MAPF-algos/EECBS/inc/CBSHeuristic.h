...
enum class EECBS_PREFIX_heuristics_type { ZERO, CG, DG, WDG, GLOBAL, PATH, LOCAL, CONFLICT, STRATEGY_COUNT }; //  GREEDY,
...
struct HTableEntry // look-up table entry 
{
	...
	struct EqNode
	{
		...
		{
			std::set<EECBS_PREFIX_Constraint> cons1[2], cons2[2];
			auto curr = h1.n;
...
{
public:
	EECBS_PREFIX_heuristics_type type;
	...
	CBSHeuristic(int num_of_agents,
							const vector<EECBS_PREFIX_Path*>& paths,
	...
	void setInadmissibleHeuristics(EECBS_PREFIX_heuristics_type h)
    {
        inadmissible_heuristic = h;
        if (h == EECBS_PREFIX_heuristics_type::CONFLICT)
        {
...
private:
    EECBS_PREFIX_heuristics_type inadmissible_heuristic;
