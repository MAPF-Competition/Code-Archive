...
int manhattanDist(const SharedEnvironment* env, int loc, int loc2){
...
void schedule_initialize(int preprocess_time_limit, SharedEnvironment* env)
{
    ...
    SchedulerE::init_heuristics(env);
    mt.seed(0);
    ...
    vector<int> valid_loc;
    for (int source = 0; source < env->map.size(); source++) {
        if (env->map[source] == 0) valid_loc.push_back(source);
    }
    int n = valid_loc.size();
    // std::cout << "Visiting size: " << valid_loc.size() << "\n";

     for (int source = 0; source < valid_loc.size() - 1; source++) {
        // if (source % 5000 == 0) std::cout << "Visiting index: " << source << "\n";
        // std::cout << "Visiting index: " << source << "\n";
        
            int source_loc = valid_loc[source];
            // int range_limit = min(n, source + 1000);
            for (int target = source + 1; target < valid_loc.size(); target++) {
                // std::cout << "Visiting target: " << valid_loc[target] << "\n";
                int target_loc = valid_loc[target]; 
                dt_get_h(env, source_loc, target_loc);
                dt_get_h(env, target_loc, source_loc);
            }
...
void schedule_plan(int time_limit, std::vector<int> & proposed_schedule,  SharedEnvironment* env)
{
    ...
    while (it_task != free_tasks.end())
    {
        ...
        for (int i = 1; i < env->task_pool[task].locations.size(); i++){
            ...
            dist_task += dt_get_hh(env, c_loc_task, next_loc_task);
            // dist_task += dt_get_hhance(c_loc_task, next_loc_task, env);
    ...
    for (auto it = min_makespan.begin(); it != min_makespan.end(); ) {
        ...
        if (env->task_pool[mm_task_id].idx_next_loc != 0) {
            ...
            // update min makespan
            it->second.second = dt_get_hh(env, env->curr_states.at(mm_agent_id).location, env->task_pool[mm_task_id].locations[0]) + cost_table[mm_task_id];
    ...
    while (!free_agents.empty())
    {
        ...
        for (auto curr : cost_table)
        {
            ...
            // dist = dt_get_hh(env, agents_finish_loc[i], first_task_loc) + curr_task_cost;
            dist = dt_get_hh(env, env->curr_states.at(i).location, first_task_loc) + curr_task_cost;
            // dist = dt_get_hhance(agents_finish_loc[i], first_task_loc, env) + curr_task_cost;
            ...
            if (!isFree) dist += dt_get_hh(env, env->curr_states.at(i).location, agents_finish_loc[i]);
    ...
    // cout << "E: Task added: " << task_added << "\n";
    // cout << "E: Assigned Task: " << assigned << "\n";
    // cout << "E: Time taken for scheduling: " << std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::steady_clock::now() - starttime).count() << "\n";
    // std::cout << "Time taken for the preprocessing: " << duration.count() << " seconds\n";
