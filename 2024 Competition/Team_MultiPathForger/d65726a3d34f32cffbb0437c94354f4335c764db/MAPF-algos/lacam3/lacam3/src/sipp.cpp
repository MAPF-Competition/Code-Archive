...
SITable::SITable(LACAM_PREFIX_CollisionTable *_CT) : CT(_CT) {}

SITable::~SITable() {}

SIs &SITable::get(LACAM_PREFIX_Vertex *v)
{
  auto &b_v = body[v->id];
...
SINode::SINode(const int _uuid, const SI &si, LACAM_PREFIX_Vertex *_v, int _t, int _g,
...
LACAM_PREFIX_Path sipp(const int i, LACAM_PREFIX_Vertex *s_i, LACAM_PREFIX_Vertex *g_i, LACAM_PREFIX_DistTable *D,
          LACAM_PREFIX_CollisionTable *CT, const Deadline *deadline, const int f_upper_bound)
{
  auto solution_path = LACAM_PREFIX_Path();
