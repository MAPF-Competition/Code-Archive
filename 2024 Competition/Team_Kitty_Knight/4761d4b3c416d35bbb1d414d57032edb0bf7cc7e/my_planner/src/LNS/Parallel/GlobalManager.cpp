...
UTIL::Timer & g_timer=UTIL::g_timer;

GlobalManager::GlobalManager(
    bool async,
    Instance & instance, std::shared_ptr<MyPlanner::Dist2PathHeuristicTable> HT, 
    std::shared_ptr<vector<float> > map_weights, std::shared_ptr<std::vector<AgentInfo> > agent_infos,
    ...
    if (!async) {
        std::cerr<<"WARNING: some functionalities might be missing in the sync version. If you want use it, please check the code and remove the following exit."<<std::endl;
        exit(-1);
...
bool GlobalManager::run(UTIL::TimeLimiter & time_limiter) {
    if (async) {
...
bool GlobalManager::_run_async(UTIL::TimeLimiter & time_limiter) {
    ...
    {
#ifndef NO_ROT
        sum_of_distances += HT->get(instance.start_locations[agent.id],instance.start_orientations[agent.id],instance.goal_locations[agent.id]);
#else
        sum_of_distances += HT->get(agent.id, instance.start_locations[agent.id]);
#endif
    }
    ...
    if (!init_neighbor.succ) {
        std::cerr << "Failed to get initial solution." << endl;
        exit(-1);
        return false;
    }
    ...
    // because already limited by the max iterations
    if (max_iterations>0 && max_iterations<INT_MAX) {
        time_limiter.disable();
    }
            ...
            #pragma omp critical
            {
                // for (int i=0;i<1;++i) {
                    ...
                    if (!time_limiter.timeout()){
                        ...
                        // including the init solution.
                        if (iteration_stats.size()>(size_t)max_iterations) {
                            stop_flag=true;
                        }
...
bool GlobalManager::_run(UTIL::TimeLimiter & time_limiter) {
...
void GlobalManager::getInitialSolution(Neighbor & neighbor) {
    ...
    for (int i=0;i<agents.size();++i) {
        ...
        if (agents[i].path.back().location!=instance.goal_locations[i] && agents[i].path.size()<=window_size_for_CT) {
            std::cerr<<"A precomputed agent "<<i<<"'s path "<<agents[i].path.size()
        ...
        if (agents[i].path.back().location!=instance.goal_locations[i] && agents[i].path.size()!=window_size_for_PATH+1) {
            std::cerr<<"we require agent either arrives at its goal earlier or has a planned path of length window_size_for_PATH. "<<agents[i].path.size()<<" vs "<<window_size_for_PATH<<endl;
            exit(-1);
        }
        ...
        // std::cerr<<agents[i].id<<" "<< agents[i].path.size()-1<<endl;
    }
