...
TimeSpaceAStarPlanner::TimeSpaceAStarPlanner(Instance & instance, std::shared_ptr<MyPlanner::Dist2PathHeuristicTable> HT, std::shared_ptr<vector<float> > weights, int execution_window): instance(instance), HT(HT), weights(weights), execution_window(execution_window) {};

void TimeSpaceAStarPlanner::findPath(int agent_idx, int start_pos, int start_orient, int goal_pos, PathTable & path_table, const UTIL::TimeLimiter & time_limiter) {
    clear();
...
#ifndef NO_ROT    
    State * start_state = new State(start_pos, start_orient, 0, 0, HT->get(agent_idx, start_pos, start_orient), 0, false, nullptr);
#else
    State * start_state = new State(start_pos, start_orient, 0, 0, HT->get(agent_idx, start_pos), 0, false, nullptr);
#endif
    ...
    while (!open_list.empty() && !time_limiter.timeout()) {
        ...
        getSuccessors(agent_idx, curr, goal_pos, path_table);
...
void TimeSpaceAStarPlanner::getSuccessors(int agent_idx, State * curr, int goal_pos, PathTable & path_table) {
    successors.clear();
    ...
    if (orient==0) {
        ...
        if (x+1<cols){
            ...
            if (map[next_pos]==0 && !path_table.constrained(pos,next_pos,next_timestep)) {
                ...
                next_h=curr->arrived?0:HT->get(agent_idx, next_pos, next_orient);
    ...
    if (x+1<cols){
        ...
        if (map[next_pos]==0 && !path_table.constrained(pos,next_pos,next_timestep)) {
            next_g=curr->g+weights[weight_idx];
            next_h=curr->arrived?0:HT->get(agent_idx, next_pos);
            next_num_of_conflicts=0;
            next_arrived=curr->arrived | (next_pos==goal_pos);
            successors.push_back(new State(next_pos, next_orient, next_timestep, next_g, next_h, next_num_of_conflicts, next_arrived, curr));
        }
