...
void PlannerLazy::init(SharedEnvironment* _env, const Parameters & parameters) {
    ...
    cout << "PlannerLazy(bound: " << m_path_bound << ") initialized" << endl;
}
...
void PlannerLazy::plan(vector<Action> & actions, double timeLeft) {
    ...
    while (/*count < 30*/ has_moved && timeLeft - elapsed_time > .07) { // @debug Change this
...
//    cout << "Planning the path of a" << agent << " from " << start.location << " to " << end << endl;
    ...
    while (!open_list.empty()) {
        ...
        auto neighborhood = neighbors(agent, Position(curr->location, curr->direction, curr->g2 + m_timestep));
...
vector<Planner::Position> PlannerLazy::neighbors(int agent, const Position &position) const {
    ...
    for (const Position & next : positions) {
        ...
        // for (int i : m_passing.at(next.location)) {
...
// OLD
// std::vector<Planner::Position> PlannerLazy::neighbors(int agent, const Position &position) const {
//     std::vector<Position> neighbors;
//     std::list<Position> positions;
//     positions.push_back(move(position, Action::W));
//     positions.push_back(move(position, Action::FW));
//     positions.push_back(move(position, Action::CR));
//     positions.push_back(move(position, Action::CCR));
//
//     const int t = position.time;
//     for (const Position & next : positions) {
//         if (next.location < 0 || obstacle(next.location))
