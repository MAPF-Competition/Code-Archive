...
void RawHeuristicMatrix::build(uint32_t source, const Graph &graph) {
    ...
    while (!heap.empty()) {
        ...
        for (uint32_t action = 0; action < 3; action++) {
            ...
            if (to && !visited[to]) {
                uint64_t to_dist = dist + 1/*graph.get_weight(to, action)*/;
...
RawHeuristicMatrix::RawHeuristicMatrix(const Graph &graph) {
    ETimer timer;
...
uint32_t RawHeuristicMatrix::get(uint32_t source, uint32_t target) const {
    if (!target) {
        return INVALID_DIST;
    }
