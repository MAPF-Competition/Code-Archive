...
int ConstraintTable::getMaxTimestep() const  // everything is static after the max timestep
{
  ...
  if (path_table_for_CT != nullptr) rst = max(rst, path_table_for_CT->makespan);
  if (path_table_for_CAT != nullptr) rst = max(rst, path_table_for_CAT->makespan);
  if (length_max < MAX_TIMESTEP) rst = max(rst, length_max);
  if (!landmarks.empty()) rst = max(rst, landmarks.rbegin()->first);
  return rst;
}

int ConstraintTable::getLastCollisionTimestep(int location) const {
  int rst = -1;
  if (path_table_for_CAT != nullptr) rst = path_table_for_CAT->getLastCollisionTimestep(location);
  if (!cat.empty()) {
    for (auto t = cat[location].size() - 1; t > rst; t--) {
      if (cat[location][t]) return t;
    }
  }
  return rst;
}
...
void ConstraintTable::insert2CAT(const Path &path) {
  ...
  for (auto timestep = (int)path.size() - 1; timestep >= 0; timestep--) {
    ...
    if (cat[loc].size() <= timestep) cat[loc].resize(timestep + 1, false);
...
bool ConstraintTable::constrained(size_t loc, int t) const {
  assert(loc >= 0);
  if (loc < map_size) {
    if (path_table_for_CT != nullptr and path_table_for_CT->constrained(loc, loc, t)) return true;
  ...
  for (const auto &constraint : it->second) {
    if (constraint.first <= t && t < constraint.second) return true;
  }
  return false;
}

bool ConstraintTable::constrained(size_t curr_loc, size_t next_loc, int next_t) const {
  return (path_table_for_CT != nullptr and
          path_table_for_CT->constrained(curr_loc, next_loc, next_t)) or
...
int ConstraintTable::getNumOfConflictsForStep(size_t curr_id, size_t next_id,
  ...
  if (!cat.empty()) {
    if (cat[next_id].size() > next_timestep and cat[next_id][next_timestep]) rst++;
    if (curr_id != next_id and cat[next_id].size() >= next_timestep and
        cat[curr_id].size() > next_timestep and cat[next_id][next_timestep - 1] and
        cat[curr_id][next_timestep])
      rst++;
    if (cat_goals[next_id] < next_timestep) rst++;
  }
  return rst;
}

bool ConstraintTable::hasConflictForStep(size_t curr_id, size_t next_id, int next_timestep) const {
  if (path_table_for_CAT != nullptr and
      path_table_for_CAT->hasCollisions(curr_id, next_id, next_timestep))
    return true;
  if (!cat.empty()) {
    if (cat[next_id].size() > next_timestep and cat[next_id][next_timestep]) return true;
    if (curr_id != next_id and cat[next_id].size() >= next_timestep and
        cat[curr_id].size() > next_timestep and cat[next_id][next_timestep - 1] and
        cat[curr_id][next_timestep])
      return true;
    if (cat_goals[next_id] < next_timestep) return true;
  }
  return false;
}
...
bool ConstraintTable::hasEdgeConflict(size_t curr_id, size_t next_id, int next_timestep) const {
  if (path_table_for_CAT != nullptr and
      path_table_for_CAT->hasEdgeCollisions(curr_id, next_id, next_timestep))
    return true;
...
int ConstraintTable::getFutureNumOfCollisions(int loc, int t) const {
  int rst = 0;
  if (path_table_for_CAT != nullptr) rst = path_table_for_CAT->getFutureNumOfCollisions(loc, t);
  if (!cat.empty()) {
...
int ConstraintTable::getHoldingTime(int location, int earliest_timestep) const {
  ...
  if (it != ct.end()) {
    for (auto time_range : it->second) rst = max(rst, time_range.second);
  }
  // Landmark
  for (auto landmark : landmarks) {
    if (landmark.second != location) rst = max(rst, (int)landmark.first + 1);
  }
