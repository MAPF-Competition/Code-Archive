...
namespace LNSSchedulerRND
{
    ...
    extern vector<int> location_counts;
    ...
    std::pair<int, std::unordered_set<int>> evaluateSchedule(const ScheduleContainer &schedule, SharedEnvironment *env, int lower_bound = INT_MAX)
    {
        int makespan = 0;
        std::unordered_set<int> critical_agents;
        std::unordered_map<int, int> agent_flow_costs;

        // マップ上の各位置での経路の重なりをカウントするための配列
        ...
        if constexpr (std::is_same_v<ScheduleContainer, std::vector<int>>)
        {
            for (int agent_id : SchedulerUtils::global_available_agents)
            {
                ...
                {
                    total_cost = SchedulerUtils::calculateOppositeFlowCost(agent_id, schedule[agent_id], env);
                }
                else
                {
                    total_cost = calculateTaskCompletionTime(agent_id, schedule[agent_id], env);
                }
                completion_time = total_cost;
                agent_flow_costs[agent_id] = completion_time;
        ...
        else
        {
            for (const auto &[agent_id, task_id] : schedule)
            {
                ...
                // パスに沿って対向流コストと混雑度コストを計算
                int total_cost;
                if (env->curr_timestep != 0)
                {
                    total_cost = SchedulerUtils::calculateOppositeFlowCost(agent_id, task_id, env);
                }
                else
                {
                    total_cost = calculateTaskCompletionTime(agent_id, task_id, env);
                }
    ...
    std::pair<int, std::unordered_set<int>> evaluateScheduleSoC(const ScheduleContainer &schedule, SharedEnvironment *env, int lower_bound = INT_MAX)
    {
        ...
        if constexpr (std::is_same_v<ScheduleContainer, std::vector<int>>)
        {
            for (int agent_id : SchedulerUtils::global_available_agents)
            {
                ...
                int total_cost = SchedulerUtils::calculateOppositeFlowCost(agent_id, schedule[agent_id], env);
        ...
        else
        {
            for (const auto &[agent_id, task_id] : schedule)
            {
                ...
                // パスに沿って対向流コストと混雑度コストを計算
                int total_cost = SchedulerUtils::calculateOppositeFlowCost(agent_id, task_id, env);
    ...
    // deadendの集合を外部から参照できるように
    extern std::unordered_set<unsigned short> deadends;
