...
namespace DefaultPlanner {
    ...
    public:
        ...
        decrease_key(s_node *val) {
            ...
            minqueue_ ? heapify_up(val->get_priority()) : heapify_down(val->get_priority());
        }

        void
        increase_key(s_node *val) {
            assert(val->get_priority() < queuesize_);
            minqueue_ ? heapify_down(val->get_priority()) : heapify_up(val->get_priority());
        }
        ...
        inline unsigned int
        size() { return queuesize_; }

        inline unsigned int
        empty() { return queuesize_ == 0; }

        inline bool
        is_minqueue() { return minqueue_; }
        ...
        size_t
        mem() {
            return maxsize_ * sizeof(s_node *) + sizeof(*this);
        }
