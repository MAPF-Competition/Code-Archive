...
bool InitLNS::run() {
  ...
  iteration_stats.emplace_back(neighbor.agents.size(), sum_of_costs, runtime, "PP", 0,
  ...
  while (runtime < time_limit and num_of_colliding_pairs > 0) {
    ...
    if (screen >= 2) cout << "New colliding pairs = " << neighbor.colliding_pairs.size() << endl;
    if (succ)  // update collision graph
    {
      num_of_colliding_pairs +=
          (int)neighbor.colliding_pairs.size() - (int)neighbor.old_colliding_pairs.size();
    ...
    if (screen >= 1)
      cout << "Iteration " << iteration_stats.size() << ", "
           ...
           << "solution cost = " << sum_of_costs << ", "
           << "remaining time = " << time_limit - runtime << endl;
    iteration_stats.emplace_back(neighbor.agents.size(), sum_of_costs, runtime, replan_algo_name, 0,
                                 num_of_colliding_pairs);
  }
...
bool InitLNS::updateCollidingPairs(set<pair<int, int>>& colliding_pairs, int agent_id,
  ...
  for (int t = 1; t < min((int)path.size(), instance.window); t++) {
    ...
    if (!path_table.goals.empty())  // target conflicts
    {
      for (const auto& goal_t : path_table.goals[to]){
        if (goal_t < t){
          // this agent traverses the target of another agent
          for (auto id : path_table.table[to][goal_t])  // look at all agents at the goal time
          {
            if (agents[id].path.back().location ==
                to)  // if agent id's goal is to, then this is the agent we want
            {
              succ = true;
              colliding_pairs.emplace(min(agent_id, id), max(agent_id, id));
              break;
  ...
  for (int t = (int)path.size(); t < min((int)path_table.table[goal].size(), instance.window);
       t++) {
...
bool InitLNS::generateNeighborByTarget() {
  ...
  agents[a].path_planner->findMinimumSetofColldingTargets(goal_table, A_target);
  // generate non-wait path and collect A_target

  if (screen >= 3) {
    cout << "     Selected a : " << a << endl;
    cout << "     Select A_start: ";
    for (auto e : A_start) cout << "(" << e.first << "," << e.second << "), ";
    cout << endl;
  ...
  neighbor.agents.assign(neighbors_set.begin(), neighbors_set.end());
  if (screen >= 2) cout << "Generate " << neighbor.agents.size() << " neighbors by target" << endl;
  return true;
}
...
bool InitLNS::generateNeighborRandomly() {
  ...
  while (neighbors_set.size() < neighbor_size) {
    ...
    for (auto i = 0; i < neighbor_size - neighbors_set.size(); i++) r[i] = rand() % total;
  ...
  neighbor.agents.assign(neighbors_set.begin(), neighbors_set.end());
  if (screen >= 2) cout << "Generate " << neighbor.agents.size() << " neighbors randomly" << endl;
  return true;
}
...
void InitLNS::writeIterStatsToFile(const string& file_name) const {
  ...
  for (const auto& data : iteration_stats) {
    output <<  // data.num_of_agents << "," <<
        data.sum_of_costs << "," << data.num_of_colliding_pairs << "," << data.runtime <<  //"," <<
        // data.algorithm <<
        endl;
  }
  output.close();
...
void InitLNS::writeResultToFile(const string& file_name, int sum_of_distances,
  ...
  stats << runtime << "," << iteration_stats.back().num_of_colliding_pairs << "," << sum_of_costs
        ...
        << iteration_stats.front().runtime << "," << auc << "," << num_LL_expanded << ","
...
void InitLNS::printPath() const {
  for (const auto& agent : agents) cout << "Agent " << agent.id << ": " << agent.path << endl;
}
...
bool InitLNS::validatePathTable() const {
  for (auto i = 0; i < agents.size(); i++) assert(path_table.getPath(i) == &agents[i].path);
  return true;
}
