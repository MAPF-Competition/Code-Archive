...
def do_segment(raw, **config):
    # If map is not already aligned, denoised and binary map, preprocess
    ...
    else: # Assume that the input is already preprocessed
        rank = raw
        binary_dilated = raw
        crop_info = (0, 0, 0, 0)
    ...
    ridges = generate_ridge_map_meijering(
        raw, **config["generate_ridge_map_meijering"])
    ...
    # G, common_borders_map, borders = find_graph_representation(segments, border_label)
    # debugger.add("G1", G)
