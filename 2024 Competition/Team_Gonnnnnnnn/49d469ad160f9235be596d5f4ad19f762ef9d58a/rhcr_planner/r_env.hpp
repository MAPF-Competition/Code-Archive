...
// Count all conflicts -hzj
  ...
  int focalHeuristic(
      ...
      std::atomic<int> numConflicts(0); // 使用原子变量来避免线程安全问题
      int max_t = 3;
      int numThreads = std::thread::hardware_concurrency(); // 获取机器核心数
      std::vector<std::thread> threads; // 存储线程

      // 对每个时间步进行并行处理
      for (int t = 0; t < max_t; ++t) {
          // 使用unordered_set缓存每个时间步的智能体位置
          std::unordered_set<std::pair<int, int>, boost::hash<std::pair<int, int>>> positions;

          auto process_task = [this, &numConflicts, &solution, &only_task_ids, t](int start, int end) {
            std::unordered_set<std::pair<int, int>, boost::hash<std::pair<int, int>>> positions; // 每个线程独立的 positions
            for (int i = start; i < end; ++i) {
                State2 state1a = getState(only_task_ids[i], solution, t);
                State2 state1b = getState(only_task_ids[i], solution, t + 1);
        
                if (positions.count(state1a.loc) > 0) {
                    ++numConflicts;
                }
                positions.insert(state1a.loc);
        
                for (size_t j = 0; j < solution.size(); ++j) {
                    if (only_task_ids[i] == j) continue;
                    State2 state2a = getState(j, solution, t);
                    State2 state2b = getState(j, solution, t + 1);
          ...
          // 将任务划分为多个块并行执行
          int numTasks = only_task_ids.size();
          int tasksPerThread = (numTasks + numThreads - 1) / numThreads; // 计算每个线程负责的任务数

          for (int i = 0; i < numThreads; ++i) {
              int start = i * tasksPerThread;
              int end = std::min((i + 1) * tasksPerThread, numTasks);
              threads.push_back(std::thread(process_task, start, end));
          }
          ...
          threads.clear(); // 清空线程队列，准备下一轮任务
      }

      return numConflicts.load(); // 返回冲突的总数量
  }
  ...
  // int focalHeuristic(
  //   const std::vector<PlanResult<State2, Action, float>>& solution, std::vector<int> & only_task_ids) {
  //   int numConflicts = 0; // 初始化冲突计数器
  ...
  //       // 检查智能体之间的顶点冲突（即同一位置）
  //       // for (size_t i = 0; i < solution.size(); ++i) {
  //       for (int i = 0; i < only_task_ids.size(); ++i) {
  //           State2 state1a = getState(only_task_ids[i], solution, t); // 获取智能体 i 在时间步 t 的状态
  //           State2 state1b = getState(only_task_ids[i], solution, t + 1); // 获取智能体 i 在时间步 t+1 的状态

  //           // 如果该位置已经被其他智能体占据，发生冲突
  //           if (positions.count(state1a.loc) > 0) {
  //               ++numConflicts; // 增加冲突计数
  //           }
  //           // 将当前智能体的状态位置加入 positions 集合
  //           positions.insert(state1a.loc);

  //           for (size_t j = 0; j < solution.size(); ++j) {
  //             State2 state2a = getState(j, solution, t); // 获取智能体 j 在时间步 t 的状态
  //             State2 state2b = getState(j, solution, t + 1); // 获取智能体 j 在时间步 t+1 的状态
  //             if (only_task_ids[i] == j)
  //             {
  //               continue;
  //             }
  //             // 检查智能体 i 和智能体 j 是否发生交换冲突
  //             if (state1a.equalExceptTime(state2b) && state1b.equalExceptTime(state2a)) {
  //                 ++numConflicts; // 如果发生交换冲突，增加冲突计数
  //             }
  //         }
  //       }
  //   }

  //   return numConflicts; // 返回冲突的总数量
  // }
...
bool getFirstConflict1(
      ...
      r_Conflict& result, std::vector<std::vector<int>>& Area_Agent)
  {
    ...
    bool getFirstConflict(
        ...
        std::atomic<bool> foundConflict(false);
        std::atomic<int> numThreads(0);
        int numThreadsTotal = std::thread::hardware_concurrency();
        ...
        for (int t = 0; t <= max_t; ++t) {
            ...
            auto process_task = [this, &solution, &only_task_ids, &result, &resultMutex, &foundConflict, t](int start, int end) {
                for (size_t i = start; i < end; ++i) {
                    if (foundConflict) return;
    
                    State2 state1a = this->getState(only_task_ids[i], solution, t);
                    State2 state1b = this->getState(only_task_ids[i], solution, t + 1);
    
                    for (size_t j = 0; j < solution.size(); ++j) {
                        if (only_task_ids[i] == j) continue;
    
                        State2 state2a = this->getState(j, solution, t);
                        State2 state2b = this->getState(j, solution, t + 1);
            ...
            int numTasks = only_task_ids.size();
            int tasksPerThread = (numTasks + numThreadsTotal - 1) / numThreadsTotal;
    
            for (int i = 0; i < numThreadsTotal; ++i) {
                int start = i * tasksPerThread;
                int end = std::min((i + 1) * tasksPerThread, numTasks);
                threads.emplace_back(process_task, start, end);
            }
...
//     // check drive-drive vertex collisions
//     // for (size_t i = 0; i < solution.size(); ++i) {
//       for (size_t i = 0; i < only_task_ids.size(); ++i) {
//       State2 state1a = getState(only_task_ids[i], solution, t);
//       State2 state1b = getState(only_task_ids[i], solution, t + 1);
//       for (size_t j = 0; j < solution.size(); ++j) {
//         if (only_task_ids[i] == j) {
...
// }
  ...
  bool getFirstConflict_modified(
      const std::vector<PlanResult<State2, Action, float> >& solution,
      r_Conflict& result,
      const std::unordered_set<size_t>& modifiedAgents) {//LHY
    int max_t = 3;

    for (int t = 0; t <= max_t; ++t) {

      // 只检查被修改的智能体
        for (auto i : modifiedAgents) 
        {
          // State2 state1 = getState(i, solution, t);
          State2 state1a = getState(i, solution, t);
          State2 state1b = getState(i, solution, t + 1);
