...
void SingleAgentSolver::findMinimumSetofColldingTargets(vector<int>& goal_table,
                                                        set<int>& A_target) {
  struct Node {
    int location;
    int g_val;
  ...
  while (!open_list.empty()) {
    ...
    for (int next_location : next_locations) {
      ...
      if (visited[next_location] == nullptr) {
        ...
        continue;
                 ...
                 (num_of_visited_targets == visited[next_location]->num_of_targets and
                  next_g_val < visited[next_location]->g_val))
      // update existing node's if needed (only in the open_list)
      {
        ...
        {
          open_list.increase(
              visited[next_location]->open_handle);  // increase because f-val improved
        }
      }
...
std::ostream& operator<<(std::ostream& os, const LLNode& node) {
  os << node.state.location << "@" << node.state.timestep << "(f=" << node.g_val << "+"
     << node.h_val << ")";
  return os;
}
