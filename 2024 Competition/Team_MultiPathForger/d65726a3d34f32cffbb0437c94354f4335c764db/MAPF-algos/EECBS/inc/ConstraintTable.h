...
public:
    int length_min = 0;
    int length_max = EECBS_MAX_TIMESTEP;
    size_t num_col;
    size_t map_size;
    ...
    void insert2CT(const list<EECBS_PREFIX_Constraint>& constraints, int agent); // insert constraints for the given agent to the constraint table
    void insert2CT(const EECBS_PREFIX_Path& path); // insert a path to the constraint table
    ...
    void insert2CAT(int agent, const vector<EECBS_PREFIX_Path*>& paths); // build the conflict avoidance table using a set of paths
    void insert2CAT(const EECBS_PREFIX_Path& path); // insert a path to the collision avoidance table
