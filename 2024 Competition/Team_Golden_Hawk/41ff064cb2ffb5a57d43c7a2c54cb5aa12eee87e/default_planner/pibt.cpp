...
int get_gp_h(TrajLNS& lns, int ai, int target){
    ...
    if (!lns.traj_dists.empty() && !lns.traj_dists[ai].empty()) // if the agent has a guide path
        min_heuristic = get_dist_2_path(lns.traj_dists[ai], lns.env, target, &(lns.neighbors));	
    else if (!lns.heuristics[lns.tasks.at(ai)].empty()) // if the goal of agent has a heuristic table
...
/**
 * @brief replan the move for each agent according to the heuristic value of each successor(possible move)
 * @param curr_id the id of the agent to be replanned(input)
 * @param higher_id the id of the agent that has higher priority(input)
 * @param prev_states the previous state of each agent(input)
 * @param next_states the next state of each agent(input)
 * @param prev_decision the decision of each agent at the previous timestep(input)
 * @param decision the decision of each agent at the next timestep(input)
 * @param occupied the occupied status of each location(input)
 * @param lns the trajectory LNS(input)
 */
bool causalPIBT(int curr_id, int higher_id,std::vector<State>& prev_states,
	...
	// sort the successors based on the heuristic value
	...
	// take next action with highest heuristic value if valid
