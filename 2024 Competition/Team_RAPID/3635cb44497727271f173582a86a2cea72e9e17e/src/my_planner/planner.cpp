#include "my_planner/planner.h"
#include "my_planner/heuristics.h"
#include "SharedEnv.h"
#include "my_planner/pibt.h"
#include "my_planner/flow.h"
...
namespace MyPlanner{
    ...
    std::vector<double> p;  //p 存储每个智能体的当前优先级（通常用来决定在冲突情况下谁具有“先手”权）；
    //p_copy 是初始的优先级备份，用于在需要重置或比较时恢复原始优先级。
    std::vector<State> prev_states;
    std::vector<State> next_states;
    std::vector<int> ids;   //一个整型向量，存储所有智能体的编号。后续会根据优先级对该向量排序，用于规划顺序。
    ...
    //一个 TrajLNS 对象，表示基于局部邻域搜索（LNS，Local Neighborhood Search）或其它启发式算法得到的引导路径数据结构。
    //它封装了智能体任务、路径信息以及相关的启发式表（heuristics）和邻居信息（global_neighbors）。
    TrajLNS trajLNS;    
    std::mt19937 mt1;
    ...
    void initialize(int preprocess_time_limit, SharedEnvironment* env){
            ...
            //对智能体编号进行随机打乱，并按照打乱后的顺序为每个智能体分配一个初始优先级值
    ...
    void plan(int time_limit,vector<Action> & actions, SharedEnvironment* env){
        ...
        //为后续 PIBT 算法（Priority Inheritance with Backtracking）预留一定的时间。
        ...
        if (env->curr_timestep == 0){
            //当当前时间步为 0 时，记录每个智能体当前所在位置作为其备用目标
        ...
        frank_wolfe(trajLNS, updated,end_time);//这一步利用 Frank-Wolfe 算法对引导路径进行进一步优化，旨在协调各智能体之间的交通流，从而减少路径冲突和拥堵。

        // sort agents based on the current priority
        //智能体排序，优先级高的智能体先规划下一步决策。
        ...
        for (int i : ids){
            ...
            if (next_states[i].location==-1){
                ...
                causalPIBT(i,-1,prev_states,next_states,
                    ...
                    occupied, trajLNS);
            }//PIBT 算法将在考虑冲突和优先级的基础上为每个智能体决定其下一步目标位置。
        }
