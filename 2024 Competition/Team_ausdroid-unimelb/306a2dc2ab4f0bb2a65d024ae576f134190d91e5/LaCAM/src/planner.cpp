...
Solution Planner::solve()
{
  ...
  while (!OPEN.empty() && !is_expired(deadline)) {
    ...
    //info(1, verbose, deadline, "find g cost: ", H->g);
    //info(1, verbose, deadline, "find h cost: ", H->h);
    ...
    // online version: find a smallest heurist value cost and seen as the suboptiaml solution
    if (H_subgoal == nullptr && is_same_config(H->C, ins->starts) == 0){
      H_subgoal = H;
    }
    if(H_subgoal != nullptr && H_subgoal->h > H->h){
      H_subgoal = H;
    }
    // check lower bounds
    if(H_subgoal != nullptr && H->f >= H_subgoal->f && H->h > H_subgoal->h){
      OPEN.pop_front();
      continue;
    }
  ...
  if(H_goal == nullptr) solution = backtrack(H_subgoal);
...
void Planner::logging()
{
  ...
  if (H_goal != nullptr && OPEN.empty()) {
    ...
    info(1, verbose, deadline, "solved sub-optimally, cost:", H_subgoal->g);
  }
