...
void Planner::read_barriers() {
    ...
    std::string fn = "data/Shadocks/assets/barriers.json";
    std::ifstream f(fn);
...
void Planner::read_distance_sites() {
    ...
    // Read the sites
    std::string json_fn = "data/Shadocks/assets/sites.json";
...
std::vector<int> Planner::agent_at(int location, int time) const {
    ...
    // agent passing
    if (m_passing_hash.find(std::make_pair(location, time))==m_passing_hash.end()) {
    ...
    if (m_ending_hash.find(location)==m_ending_hash.end()) {
