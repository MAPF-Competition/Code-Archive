...
shared_ptr<EECBS_PREFIX_Conflict> RectangleReasoning::run(const vector<EECBS_PREFIX_Path*>& paths, int timestep,
...
shared_ptr<EECBS_PREFIX_Conflict> RectangleReasoning::findRectangleConflictByGR(const vector<EECBS_PREFIX_Path*>& paths, int timestep,
	int a1, int a2, const MDD* mdd1, const MDD* mdd2)
{
	assert(timestep > 0);
	...
	list<EECBS_PREFIX_Constraint> B1;
	...
	list<EECBS_PREFIX_Constraint> B2;
	...
	list<EECBS_PREFIX_Constraint> constraint1;
	list<EECBS_PREFIX_Constraint> constraint2;
	bool succ;
	...
	if (!blocked(*paths[a1], constraint1) || !blocked(*paths[a2], constraint2))
		return nullptr;
	auto rectangle = std::make_shared<EECBS_PREFIX_Conflict>();
...
shared_ptr<EECBS_PREFIX_Conflict> RectangleReasoning::findRectangleConflictByRM(const vector<EECBS_PREFIX_Path*>& paths, int timestep,
        int a1, int a2, const MDD* mdd1, const MDD* mdd2)
{
    shared_ptr<EECBS_PREFIX_Conflict> rectangle = nullptr;
                ...
                for (int t2_end : g2s)
                {
                    ...
                    if (new_type > type || (new_type == type && new_area > area))
                    {
                        ...
                        if (succ && blocked(*paths[a1], constraint1) && blocked(*paths[a2], constraint2))
                        {
							type = new_type;
							area = new_area;
							rectangle = std::make_shared<EECBS_PREFIX_Conflict>();
							rectangle->rectangleConflict(a1, a2, Rs, Rg, Rg_t, constraint1, constraint2);
                            if (type == 2)
							{
								rectangle->priority = conflict_priority::CARDINAL;
...
bool RectangleReasoning::ExtractBarriers(const MDD& mdd, int loc, int timestep,
					int dir1, int dir2, int start, int goal, int start_time, list<EECBS_PREFIX_Constraint>& B)
{
	int num_barrier;
	...
	vector<int> extent_L(num_barrier, EECBS_MAX_TIMESTEP);
	...
	for (size_t t = 1; t < mdd.levels.size(); t++)
	{
		for (auto n : mdd.levels[t])
		{
			...
			{
				if (n->children.size() == 1 && extent_L[barrier_id] == EECBS_MAX_TIMESTEP &&
...
bool RectangleReasoning::isEntryBarrier(const EECBS_PREFIX_Constraint& b1, const EECBS_PREFIX_Constraint& b2, int dir1)
{
...
bool RectangleReasoning::isExitBarrier(const EECBS_PREFIX_Constraint& b1, const EECBS_PREFIX_Constraint& b2, int dir1)
{
...
pair<int, int> RectangleReasoning::getIntersection(const EECBS_PREFIX_Constraint& b1, const EECBS_PREFIX_Constraint& b2)
{
	pair<int, int> b1_l = instance.getCoordinate(get<1>(b1));
	pair<int, int> b1_u = instance.getCoordinate(get<2>(b1));
	pair<int, int> b2_l = instance.getCoordinate(get<1>(b2));
...
bool RectangleReasoning::isCut(const EECBS_PREFIX_Constraint& b, const pair<int, int>& Rs, const pair<int, int>& Rg)
{
	...
	for (const auto& b1_entry : B1)
	{
		for (const auto& b2_entry : B2)
		{
			if (isEntryBarrier(b1_entry, b2_entry, dir1) && isEntryBarrier(b2_entry, b1_entry, dir2))
			{
				...
				list<EECBS_PREFIX_Constraint>::const_reverse_iterator  b1_exit = B1.rbegin();
				list<EECBS_PREFIX_Constraint>::const_reverse_iterator  b2_exit = B2.rbegin();
...
list<int> RectangleReasoning::getStartCandidates(const EECBS_PREFIX_Path& path, const MDD& mdd, int timestep)
{
	list<int> starts;
...
list<int> RectangleReasoning::getGoalCandidates(const EECBS_PREFIX_Path& path, const MDD& mdd, int timestep)
{
	list<int> goals;
...
int RectangleReasoning::getStartCandidate(const EECBS_PREFIX_Path& path, int dir1, int dir2, int timestep)
{
...
int RectangleReasoning::getGoalCandidate(const EECBS_PREFIX_Path& path, int dir1, int dir2, int timestep)
{
...
bool RectangleReasoning::addModifiedBarrierConstraints(int a1, int a2, 
	...
	const MDD* mdd1, const MDD* mdd2,
	list<EECBS_PREFIX_Constraint>& constraint1, list<EECBS_PREFIX_Constraint>& constraint2)
{
...
bool RectangleReasoning::blocked(const EECBS_PREFIX_Path& path, const list<EECBS_PREFIX_Constraint>& constraints)
{
...
bool RectangleReasoning::traverse(const EECBS_PREFIX_Path& path, int loc, int t)
{
	if (t >= (int)path.size())
