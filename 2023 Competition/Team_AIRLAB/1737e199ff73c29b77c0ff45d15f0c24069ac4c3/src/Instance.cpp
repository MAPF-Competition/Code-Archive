...
Instance::Instance(SharedEnvironment* env, int window) {
  this->env = env;
  this->window = window;
}
  ...
  while (start < 3 && goal < 4) {
    if (x[start] < 0 || x[start] >= num_of_rows || y[start] < 0 || y[start] >= num_of_cols ||
      ...
      goal++;
                         ...
                         linearizeCoordinate(x[goal], y[goal])))
...
void Instance::generateConnectedRandomGrid(int rows, int cols, int obstacles) {
  cout << "Generate a " << rows << " x " << cols << " grid with " << obstacles << " obstacles. "
       << endl;
  int i, j;
  num_of_rows = rows + 2;
  num_of_cols = cols + 2;
...
void Instance::printAgents() const {
  for (int i = 0; i < num_of_agents; i++) {
    cout << "Agent" << i << " : S=(" << getRowCoordinate(start_states[i].location) << ","
         << getColCoordinate(start_states[i].location) << ") ; G=("
         << getRowCoordinate(goal_locations[i]) << "," << getColCoordinate(goal_locations[i]) << ")"
         << endl;
  }
}
...
void Instance::saveAgents() const {
  ...
  for (int i = 0; i < num_of_agents; i++)
    myfile << getRowCoordinate(start_states[i].location) << ","
...
void Instance::saveNathan() const {
  ...
  for (int i = 0; i < num_of_agents; i++)
    myfile << i << "\t" << map_fname << "\t" << this->num_of_cols << "\t" << this->num_of_rows
           << "\t" << getColCoordinate(start_states[i].location) << "\t"
           << getRowCoordinate(start_states[i].location) << "\t"
           << getColCoordinate(goal_locations[i]) << "\t" << getRowCoordinate(goal_locations[i])
           << "\t" << 0 << endl;
  myfile.close();
}
...
enum Orientation { EAST = 0, SOUTH = 1, WEST = 2, NORTH = 3 };
...
list<State> Instance::getNeighbors(State curr_state) const {
  ...
  for (auto &next : candidates) {
    if (validMove(curr_state.location, next.location)) neighbors.emplace_back(next);
  }
  return neighbors;
}
  ...
  for (auto i = 0; i < start_states.size(); i++) {
    if (paths[i] == nullptr or paths[i]->empty()) {
      ...
      exit(-1);
               ...
               start_states[i].orientation != paths[i]->front().orientation) {
    ...
    for (int t = 1; t < (int)paths[i]->size(); t++) {
      if (!validMove(paths[i]->at(t - 1).location, paths[i]->at(t).location)) {
        cerr << "The path of agent " << i << " jumps from " << paths[i]->at(t - 1).location
             << " to " << paths[i]->at(t).location << " between timesteps " << t - 1 << " and " << t
             << endl;
        exit(-1);
      }
    }
  ...
  for (auto i = 0; i < start_states.size(); i++) {
    for (auto j = i + 1; j < start_states.size(); j++) {
      ...
      for (; t < min((int)paths[a1]->size(), window); t++) {
        ...
        {
          if (num_of_colliding_pairs == 0) {
            cerr << "Find an edge conflict between agents " << a1 << " and " << a2 << " at edge ("
      ...
      if (!found_collision) {
        ...
        for (; t < min((int)paths[a2]->size(), window); t++) {
          ...
          {
            if (num_of_colliding_pairs == 0) {
              cerr << "Find a target conflict where agent " << a2 << " (of length "
