...
bool PIBT::funcPIBT(const int i, const Config &Q_from, Config &Q_to)
{
  ...
  _Vertex *prioritized_vertex = nullptr;
  if (scatter != nullptr) {
  ...
  std::sort(C_next[i].begin(), C_next[i].begin() + K + 1,
            [&](_Vertex *const v, _Vertex *const u) {
...
bool PIBT::is_swap_required(const int pusher, const int puller,
                            _Vertex *v_pusher_origin, _Vertex *v_puller_origin)
{
...
bool PIBT::is_swap_possible(_Vertex *v_pusher_origin, _Vertex *v_puller_origin)
{
  // simulate pull
  auto v_pusher = v_pusher_origin;
  auto v_puller = v_puller_origin;
