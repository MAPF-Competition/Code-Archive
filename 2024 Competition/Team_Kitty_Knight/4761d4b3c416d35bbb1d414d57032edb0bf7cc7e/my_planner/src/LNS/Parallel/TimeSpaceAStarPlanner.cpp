...
TimeSpaceAStarPlanner::TimeSpaceAStarPlanner(Instance & instance, std::shared_ptr<TrafficMAPF::Dist2PathHeuristicTable> HT, std::shared_ptr<vector<float> > weights, int execution_window): instance(instance), HT(HT), weights(weights), execution_window(execution_window) {};

void TimeSpaceAStarPlanner::findPath(int agent_idx, int start_pos, int start_orient, int goal_pos, PathTable & path_table, const TimeLimiter & time_limiter) {
    clear();
