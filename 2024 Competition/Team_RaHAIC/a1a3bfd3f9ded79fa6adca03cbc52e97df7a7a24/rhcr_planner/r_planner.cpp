    ...
    void plan(int time_limit,vector<Action> & actions,  SharedEnvironment* env)
    {
         ...
         for(int i=0; i<env->num_of_agents; i++)
        {
            ...
            if (env->goal_locations[i].empty()){
                ...
                // if (i == 97)
                // {
                //     std::cout << "A-RHCR Initial Goals Agent " << i << "(" << goals[i].x << "," << goals[i].y << ")" << std::endl;
                // }
            }
            else{
        ...
        // std::cout << "Agent 97 的起始位置和目标位置: " << std::endl;
        // // 打印 agent97 的起始位置和目标位置
        // int agent97 = 97;
        // if (agent97 < env->num_of_agents) {
        //     // 打印 Agent 97 起始位置
        //     const State2& startState97 = startStates[agent97];
        //     std::cout << "Agent 97 Start Location: (" 
        //             << startState97.loc.first << ", " 
        //             << startState97.loc.second << ") at timestep " 
        //             << startState97.timestep << std::endl;

        //     // 打印 Agent 97 目标位置
        //     const Location& goalLoc97 = goals[agent97];
        //     std::cout << "Agent 97 Goal Location: (" 
        //             << goalLoc97.x << ", " 
        //             << goalLoc97.y << ")" << std::endl;
        // }
        ...
        for (int i = 0; i < ids.size(); i++)
        {
            if (activeAgents.count(i) == 0) {
                actions.at(i) = Action::W;  // 非活跃智能体保持等待
                continue;
            }
            ...
            //     next_states2[i] = solution[i].states[0].first;
            // } else {
            //     next_states2[i] = solution[i].states[1].first; 
            // }
            // if (solution[i].states.size() == 1){
            //     std::cout << "solution[i].states.size() == 1 "<< i <<std::endl;
            //     continue;
            // }
        ...
        // 将next_states2更新到actions中  --> 应该是这里有问题，没有更新明白

        prev_states2 = next_states2;
        return;
    };
