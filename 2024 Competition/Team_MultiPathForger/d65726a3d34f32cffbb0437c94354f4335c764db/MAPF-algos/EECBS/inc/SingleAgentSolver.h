...
public:
	...
	virtual EECBS_PREFIX_Path findOptimalPath(const HLNode& node, const ConstraintTable& initial_constraints,
		const vector<EECBS_PREFIX_Path*>& paths, int agent, int lower_bound) = 0;
	virtual pair<EECBS_PREFIX_Path, int> findSuboptimalPath(const HLNode& node, const ConstraintTable& initial_constraints,
		const vector<EECBS_PREFIX_Path*>& paths, int agent, int lowerbound, double w) = 0;  // return the path and the lowerbound
	...
	SingleAgentSolver(const EECBS_PREFIX_Instance& instance, int agent) :
