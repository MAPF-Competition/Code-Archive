...
int get_manhattan(SharedEnvironment* env, int loc1, int loc2) {
...
void schedule_initialize(int preprocess_time_limit, SharedEnvironment* env)
{
    ...
    SchedulerD::init_heuristics(env);
    mt.seed(0);
...
void schedule_plan(int time_limit, std::vector<int> & proposed_schedule,  SharedEnvironment* env)
{
    ...
    while (it_task != free_tasks.end())
    {
        ...
        for (int i = 1; i < env->task_pool[task].locations.size(); i++){
            ...
            dist_task += get_manhattan(env, c_loc_task, next_loc_task);
    ...
    for (auto it = min_makespan.begin(); it != min_makespan.end(); ) {
        ...
        if (env->task_pool[mm_task_id].idx_next_loc != 0) {
            ...
            // update min makespan
            it->second.second = get_manhattan(env, env->curr_states.at(mm_agent_id).location, env->task_pool[mm_task_id].locations[0]) + cost_table[mm_task_id];
    ...
    while (!free_agents.empty())
    {
        ...
        for (auto curr : cost_table)
        {
            ...
            dist = get_manhattan(env, agents_finish_loc[i], first_task_loc) + curr_task_cost;
    ...
    // cout << "D: Task added: " << task_added << "\n";
    // cout << "D: Assigned Task: " << assigned << "\n";
    // cout << "D: Time taken for scheduling: " << std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::steady_clock::now() - starttime).count() << "\n";
