...
namespace ParallelLNSSchedulerWeighted
{
                ...
                SharedData &shared_data)
    {
        ...
        int max_count = LNSSchedulerWeighted::disable_agents ? schedule.size() : INT_MAX;
        ...
        for (int agent_id : destroyed_agents)
        {
            ...
            std::vector<LNSSchedulerWeighted::TaskCost> task_costs;
            {
            ...
            if (task_costs.empty())
            {
                ...
                {
                    if (LNSSchedulerWeighted::count_deadends && SchedulerUtils::hasDeadEndInErrand(task_id, env))
                    {
                    ...
                    WeightedCostType cost = LNSSchedulerWeighted::calculateTaskCompletionTime(agent_id, task_id, env);
                    if (task_id == current_task_id)
                    {
                        cost += LNSSchedulerWeighted::same_task_additional_cost;
                    }
                    else if (env->task_pool[task_id].agent_assigned != -1)
                    {
                        cost += LNSSchedulerWeighted::assinged_task_additional_cost;
                    }
                    if (LNSSchedulerWeighted::INCLUDE_FLOW_COST)
                    {
                                                  ...
                                                  SharedData &shared_data)
    {
        ...
        {
            std::vector<int> tmp_nearby_agents = LNSSchedulerWeighted::getNearbyAgents(agent_id, LNSSchedulerWeighted::MANHATTAN_DISTANCE_THRESHOLD, critical_agents, env);
        ...
        while (std::chrono::steady_clock::now() < end_time && iteration < max_iterations && failed_attempts < max_failed_attempts)
        {
            ...
            std::vector<int> destroyed_agents = LNSSchedulerWeighted::destroy(current_destroy_ratio, critical_agents, nearby_agents, best_schedule);
            ...
            // 評価
            auto [new_makespan, new_critical_agents] = LNSSchedulerWeighted::evaluateSchedule(tmp_schedule, env, best_makespan);
            ...
            if (new_makespan < best_makespan)
            {
                ...
                std::tie(best_makespan, critical_agents) = LNSSchedulerWeighted::evaluateSchedule(best_schedule, env);
                nearby_agents.clear();
                for (int agent_id : critical_agents)
                {
                    std::vector<int> tmp_nearby_agents = LNSSchedulerWeighted::getNearbyAgents(agent_id, LNSSchedulerWeighted::MANHATTAN_DISTANCE_THRESHOLD, critical_agents, env);
                                             ...
                                             SharedData &shared_data)
    {
        ...
        while (std::chrono::steady_clock::now() < end_time && iteration < max_iterations && failed_attempts < max_failed_attempts)
        {
            ...
            auto [current_soc, _] = LNSSchedulerWeighted::evaluateScheduleSoC(tmp_schedule, env);
            ...
            // 評価
            auto [new_soc, __] = LNSSchedulerWeighted::evaluateScheduleSoC(tmp_schedule, env, current_soc);
            ...
            if (new_soc < current_soc)
            {
                ...
                std::tie(best_soc, critical_agents) = LNSSchedulerWeighted::evaluateScheduleSoC(best_schedule, env);
                nearby_agents.clear();
                for (int agent_id : critical_agents)
                {
                    std::vector<int> tmp_nearby_agents = LNSSchedulerWeighted::getNearbyAgents(agent_id, LNSSchedulerWeighted::MANHATTAN_DISTANCE_THRESHOLD, critical_agents, env);
                       ...
                       SharedEnvironment *env)
    {
        ...
        LNSSchedulerWeighted::resetTaskSwitches(env);
        // 初期スケジュールを生成
        // auto initial_result = LNSSchedulerWeighted::generateInitialSchedule(proposed_schedule, env, end_time);
        LNSSchedulerWeighted::generateInitialScheduleQuick(proposed_schedule, env, end_time);
        ...
        std::unordered_map<unsigned short, std::vector<LNSSchedulerWeighted::TaskCost>> agent_task_costs;
        // 共有データの準備
        ...
        {
            std::tie(initial_objective, initial_critical_agents) = LNSSchedulerWeighted::evaluateScheduleSoC(proposed_schedule, env);
        }
        else
        {
            std::tie(initial_objective, initial_critical_agents) = LNSSchedulerWeighted::evaluateSchedule(proposed_schedule, env);
        }
        ...
        LNSSchedulerWeighted::updateTaskSwitches(proposed_schedule, env);
    }
}