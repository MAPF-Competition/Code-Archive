...
vector<Planner::Position> PlannerLazy::neighbors(int agent, const Position &position) const {
    ...
    for (const Position & next : positions) {
        ...
        for(int tt = t; tt <= t+1; tt++)
            if(m_passing_hash.find(std::make_pair(next.location,tt))==m_passing_hash.end())
        ...
        if(m_ending_hash.find(next.location)==m_ending_hash.end())
...
std::vector<Planner::Position> PlannerLazy::neighbors_v4(int agent, const Position &position) const {
    ...
    if (fw_loc >= 0 && !obstacle(fw_loc)) {   // Conflicts when movin forward
        bool valid = true;
        if(m_ending_hash.find(fw_loc)==m_ending_hash.end()) {
        ...
        if(m_passing_hash.find(std::make_pair(fw_loc, t+1))==m_passing_hash.end()) {
        ...
        // Edge conflicts
        if(valid && m_passing_hash.find(std::make_pair(fw_loc, t))==m_passing_hash.end()) {
    ...
    { // Conflicts when staying in the same location
        ...
        if(m_ending_hash.find(stay_loc)==m_ending_hash.end()) {
        ...
        if(valid && m_passing_hash.find(std::make_pair(stay_loc, t+1))==m_passing_hash.end()) {
...
void PlannerLazy::avoid_wait_collisions(vector<Action> & actions) {
    ...
    for(std::pair<int,int> pos_a : movingAgents) {
        ...
        int a = pos_a.second;
        while(a != -1 && waitingLocations.find(nextLocation)==waitingLocations.end()) {
            ...
            if(movingAgents.find(previousLocation)==movingAgents.end()) {
