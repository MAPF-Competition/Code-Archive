...
public:
    ...
    bool *closed_list[32];
    GamePlanner(SharedEnvironment *env) : env(env){};
    virtual ~GamePlanner() {
        delete [] mapFree; 
        delete [] freeCells;
        delete [] nebors;
    ...
    void single_agent_plan(int thread_id, int start, int start_direct, int end, std::deque<pair<int, int>> &path);
    ...
    int shift_id[33], max_time_limit;   
    ...
    int * out_nebors_cnt;
    void genAllNeighors();
    int * freeCells, * row, * col, * nebors, *emergency_nebors, *costEdge, *priority, *priority_branch;
