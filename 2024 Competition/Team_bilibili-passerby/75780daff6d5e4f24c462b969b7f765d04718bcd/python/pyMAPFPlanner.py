...
class pyMAPFPlanner:
    ...
    def initialize(self, preprocess_time_limit: int):
        ...
        print('--------')
        self.policy_model = CTDETeamPolicy(cfg).to('cuda:0')
        restore_ckp_model(self.policy_model, self.env.file_storage_path + "/simple_random_10_model_20241028.ckpt", device='cuda:0')
    ...
    def plan(self, time_limit):
        ...
        all_actions, self.cur_lstm_states = self.policy_model.action(torch.tensor(observations, dtype=torch.float32).to('cuda:0'),
                                                                     torch.tensor(infos['action_mask'],dtype=torch.float32).to('cuda:0'),
                                                                     torch.tensor(self.cur_lstm_states, dtype=torch.float32).to('cuda:0'))
        ...
        # else:
        #     all_actions[0] = MAPF.Action.FW
        print(self.ep_step, ': all actions --------->', all_actions[0], time_remaining, "all rewards --->", raw_rewards[0], 'action mask --->', infos['action_mask'][0], self.agent_positions[0])
    ...
    def step(self):
        ...
        if len(self.env.errors) > 0:
            if self.env.errors[-1][-1] == self.ep_step:
                ...
                raw_rewards[agent1_id] -= 20
                raw_rewards[agent2_id] -= 20
                self.conflict += 1
