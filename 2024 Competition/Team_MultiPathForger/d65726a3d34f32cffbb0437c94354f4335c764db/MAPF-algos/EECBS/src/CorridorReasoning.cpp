...
shared_ptr<EECBS_PREFIX_Conflict> CorridorReasoning::run(const shared_ptr<EECBS_PREFIX_Conflict>& conflict,
...
int CorridorReasoning::findCorridor(const shared_ptr<EECBS_PREFIX_Conflict>& conflict,
...
shared_ptr<EECBS_PREFIX_Conflict> CorridorReasoning::findCorridorConflict(const shared_ptr<EECBS_PREFIX_Conflict>& conflict,
	const vector<EECBS_PREFIX_Path*>& paths, const HLNode& node)
{
	...
	t3 = search_engines[conflict->a1]->getTravelTime(paths[conflict->a1]->front().location, u[1], ct1, EECBS_MAX_TIMESTEP);
	ct1.insert2CT(edge.first, edge.second, 0, EECBS_MAX_TIMESTEP); // block the corridor in both directions
	ct1.insert2CT(edge.second, edge.first, 0, EECBS_MAX_TIMESTEP);
	...
	t4 = search_engines[conflict->a2]->getTravelTime(paths[conflict->a2]->front().location, u[0], ct2, EECBS_MAX_TIMESTEP);
	ct2.insert2CT(edge.first, edge.second, 0, EECBS_MAX_TIMESTEP); // block the corridor in both directions
	ct2.insert2CT(edge.second, edge.first, 0, EECBS_MAX_TIMESTEP);
    ...
    if (abs(t3 - t4) <= corridor_length && t3_ > t3 && t4_ > t4)
    {
        ...
        shared_ptr<EECBS_PREFIX_Conflict> corridor = make_shared<EECBS_PREFIX_Conflict>();
...
bool CorridorReasoning::blocked(const EECBS_PREFIX_Path& path, const EECBS_PREFIX_Constraint& constraint)
{
	int a, loc, t1, t2;
	constraint_type type;
