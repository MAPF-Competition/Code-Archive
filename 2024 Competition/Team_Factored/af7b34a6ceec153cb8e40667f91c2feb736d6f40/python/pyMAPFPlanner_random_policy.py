...
fhandler = logging.FileHandler(filename=f'{__name__}_random.log', mode='w')
...
class pyMAPFPlanner:
    ...
    def plan(self, time_limit):
        ...
        available_actions = [MAPF.Action.W, MAPF.Action.FW, MAPF.Action.CR, MAPF.Action.CCR]
        available_actions = [MAPF.Action.W, MAPF.Action.CR, MAPF.Action.CCR]
        num_agents = self.env.num_of_agents
        actions = np.random.choice(available_actions, num_agents)
        return actions
    ...
    def sample_priority_planner(self,time_limit:int):
        ...
        for i in range(self.env.num_of_agents):
            path = []
            logger.debug(f"Agent {i}: {self.env.goal_locations[i]=}")

            if not self.env.goal_locations[i]:
                # No goal assigned
                # Stay at the same postiion
                current_path = (self.env.curr_states[i].location, self.env.curr_states[i].orientation)
                current_reservation = (self.env.curr_states[i].location, -1, 1)
                path.append(current_path)
                reservation.add(current_reservation)
                logger.debug(f"Agent {i}: {current_path=}")
                logger.debug(f"Agent {i}: {current_reservation=}")
