...
void CBSHeuristic::updateInadmissibleHeuristics(HLNode& curr)
{
	...
	{
	case EECBS_PREFIX_heuristics_type::PATH:
	...
	case EECBS_PREFIX_heuristics_type::GLOBAL: // Note: there is no "break" in the previous line, so here we compute heuristics for both GLOBAL and PATH
		...
		break;
	case EECBS_PREFIX_heuristics_type::LOCAL:
		...
		break;
	case EECBS_PREFIX_heuristics_type::CONFLICT:
...
{
    if ((inadmissible_heuristic == EECBS_PREFIX_heuristics_type::GLOBAL ||
         inadmissible_heuristic == EECBS_PREFIX_heuristics_type::PATH ||
         inadmissible_heuristic == EECBS_PREFIX_heuristics_type::LOCAL ||
         inadmissible_heuristic == EECBS_PREFIX_heuristics_type::CONFLICT) && curr.parent != nullptr)
	{
		...
		if (curr.parent->fully_expanded) // update error
		{
			...
			if (inadmissible_heuristic == EECBS_PREFIX_heuristics_type::GLOBAL) // update global error
			{
				sum_distance_errors[0] += curr.parent->distance_error;
				sum_cost_errors[0] += curr.parent->cost_error;
				num_of_errors[0]++;
			}
			else if (inadmissible_heuristic == EECBS_PREFIX_heuristics_type::LOCAL) // update local error
			{
				num_of_errors[0]++;
			...
			else if (inadmissible_heuristic == EECBS_PREFIX_heuristics_type::CONFLICT) // update conflict error
            {
...
{
    if (inadmissible_heuristic == EECBS_PREFIX_heuristics_type::ZERO)
        return;
	// Find the best child
...
bool CBSHeuristic::computeInformedHeuristics(CBSNode& curr, double _time_limit)
{
	...
	/*if (type == EECBS_PREFIX_heuristics_type::WDG) //compute distance-to-go heuristics
	{
...
pair<int, int> CBSHeuristic::solve2Agents(int a1, int a2, const CBSNode& node, bool cardinal)
{
	...
	cbs.setHeuristicType(EECBS_PREFIX_heuristics_type::CG, EECBS_PREFIX_heuristics_type::ZERO);
...
int CBSHeuristic::getEdgeWeight(int a1, int a2, CBSNode& node, bool cardinal)
{
	...
	if (type != EECBS_PREFIX_heuristics_type::CG)
	{
	...
	if (cardinal)
		...
		(type == EECBS_PREFIX_heuristics_type::DG || type == EECBS_PREFIX_heuristics_type::WDG))
	{
		// get mdds
	...
	if (type == EECBS_PREFIX_heuristics_type::WDG && rst > 0)
	{
		...
		cbs.setPrioritizeConflicts(PC);
		cbs.setHeuristicType(EECBS_PREFIX_heuristics_type::CG);
		cbs.setDisjointSplitting(disjoint_splitting);
		cbs.setBypass(false); // I guess that bypassing does not help two-agent path finding???
		cbs.setRectangleReasoning(rectangle_reasoning);
		cbs.setCorridorReasoning(corridor_reasoning);
...
{
	//copy conflict graph
	if (type == EECBS_PREFIX_heuristics_type::DG || type == EECBS_PREFIX_heuristics_type::WDG)
	{
