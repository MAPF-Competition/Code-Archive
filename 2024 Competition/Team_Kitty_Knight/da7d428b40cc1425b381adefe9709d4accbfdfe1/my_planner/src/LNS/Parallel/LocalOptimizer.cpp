...
LocalOptimizer::LocalOptimizer(
    Instance & instance, std::vector<Agent> & agents, std::shared_ptr<MyPlanner::Dist2PathHeuristicTable> HT, 
...
void LocalOptimizer::optimize(Neighbor & neighbor, const UTIL::TimeLimiter & time_limiter) {
    ...
    {
        std::cerr << "Wrong replanning strategy" << endl;
        exit(-1);
    }
    if (!succ) {
        if (screen>=1)
            std::cerr<<"replan failed"<<endl;
    }
...
bool LocalOptimizer::runPP(Neighbor & neighbor, const UTIL::TimeLimiter & time_limiter)
{
    ...
    while (p != shuffled_agents.end()) {
        ...
        if (search_priority==1) {
            ...
            path_planner->findPath(id, start_pos,start_orient, goal_pos, path_table, time_limiter);
