...
bool PBS::generate_root_node()
{
    ...
    // cout << 564 << endl;
    // Path path = path_planner.run(G, starts[10], goal_locations[10], rt);
    // starts[10].location += 1;
    // goal_locations[10][0].first += 2;
    for (int i = 0; i < num_of_agents; i++)
    {
        // cout << "i: " << i << endl;
        if (paths[i] != nullptr)
        ...
        t = std::clock();
        // cout << 581 << endl;
        // cout << "start i: " << starts[i].location << " " << starts[i].timestep << " " << starts[i].orientation << endl;
        // cout << "goal i: " << goal_locations[i][0].first << " " << goal_locations[i][0].second << endl;
...
void PBS::print_results() const
{
    ...
    std::cout << runtime << "," <<
              HL_num_expanded << "," << HL_num_generated << ", cost " <<
