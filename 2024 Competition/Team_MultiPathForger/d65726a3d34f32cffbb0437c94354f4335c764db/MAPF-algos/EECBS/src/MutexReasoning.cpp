...
shared_ptr<EECBS_PREFIX_Conflict> MutexReasoning::run(int a1, int a2, CBSNode& node, MDD* mdd_1, MDD* mdd_2)
{
	clock_t t = clock();
...
shared_ptr<EECBS_PREFIX_Conflict> MutexReasoning::findMutexConflict(int a1, int a2, CBSNode& node, MDD* mdd_1, MDD* mdd_2){
  ...
  shared_ptr<EECBS_PREFIX_Conflict> mutex_conflict = nullptr;
  ...
  if (mutex_conflict == nullptr){
    // generate constraint;
    mutex_conflict = make_shared<EECBS_PREFIX_Conflict>();
    ...
    // mutex_conflict->constraint1 = list<EECBS_PREFIX_Constraint>(a.begin(), a.end());
    // mutex_conflict->constraint2 = list<EECBS_PREFIX_Constraint>(b.begin(), b.end());

    lookupTable[c_1][c_2] = mutex_conflict;
  }

  // prepare for return
  shared_ptr<EECBS_PREFIX_Conflict> conflict_to_return = make_shared<EECBS_PREFIX_Conflict>(*mutex_conflict);
