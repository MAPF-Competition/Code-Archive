...
void CUDAHeuristics::setStateNum(int stateNum, SharedEnvironment *env)
{
  ...
  {
    CHECK_RUNTIMING_API(cudaMalloc((void **)&freeHeuristictableDev[i], intSize * freeHeuristictableDevLen * savedSizeHost));
  }
}
...
void CUDAHeuristics::copy2cuda()
{
  ...
  for (int i = 0; i < free_state_num; i++)
  {
    ...
    locDevice(transferHMapDev, orientation_heuristictable[i].data(), intSize * savedSizeHost);
    locDevice(transferIdxDev, &i, intSize);
    int section_id = i / freeHeuristictableDevLen;
...
thrust::host_vector<int> CUDAHeuristics::schedule(std::vector<int> &agentsHost, std::vector<int> &freeTasksID, std::vector<int> &unfinished_agent_cost)
{
  CHECK_RUNTIMING_API(cudaSetDevice(device_id));
  ...
  int unfinished_num = unfinished_agent_cost.size();
  int *unfinishedAgentCostDev, *unfinishedNumDev;
  if (unfinished_num > 0)
  {
    locDevice(unfinishedAgentCostDev, unfinished_agent_cost.data(), intSize * unfinished_num);
    locDevice(unfinishedNumDev, &unfinished_num, intSize);
    dim3 grid = calGrid(unfinished_num * taskNum);
    updateUnfinished<<<grid, BLOCK>>>(distDev, agentsLocDev, agentIDsDev, s_ATNumDev, savedSizeDev, unfinishedAgentCostDev, unfinishedNumDev);
  }
  ...
  // thrust::host_vector<int> minValuesHost(dist.begin(), dist.end());
  ...
  if (unfinished_num > 0)
  {
    CHECK_RUNTIMING_API(cudaFree(unfinishedAgentCostDev));
    CHECK_RUNTIMING_API(cudaFree(unfinishedNumDev));
  }
