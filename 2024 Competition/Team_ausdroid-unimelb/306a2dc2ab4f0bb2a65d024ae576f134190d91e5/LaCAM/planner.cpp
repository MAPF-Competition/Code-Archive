...
namespace LaCAMPlanner{
    ...
    void initialize(int preprocess_time_limit, SharedEnvironment* env){
            ...
            //my data structures
            start_indexes.resize(env->num_of_agents);
    ...
    void plan(int time_limit,vector<Action> & actions, SharedEnvironment* env){
        ...
        //     start_indexes[i] = env->curr_states.at(i).location;
        //     goal_indexes[i] = env->goal_locations[i][0].first;
        ...
        // Instance instance = Instance("./example_problems/random.domain/maps/random-32-32-20.map", start_indexes, goal_indexes);
        // Deadline deadline = Deadline(470);


        // Solution solution = solve(instance, 1, &deadline, 123);

        // if(solution.size() != 0){
        //     for(int i = 0; i<env->num_of_agents;i++){
        //         decided.at(i) = DCR({solution[1][i]->index,DONE::NOT_DONE});
        //         actions.at(i) = getAction(prev_states.at(i),decided.at(i).loc, env);
        //         checked.at(i) = false;
        //         // std::cout <<"agents id:" << i << endl; 
        //         // std::cout <<"prev_states:" << prev_states[i] << endl; 
        //         // std::cout <<"goal_indexes:"<< goal_indexes[i] << endl;
        //         // std::cout <<"decided:" << decided.at(i).loc << endl; 
        //         // std::cout <<"actions:" << actions.at(i) << endl; 
        //         // std::cout << " "<< endl; 
        ...
        for (int i : ids){
            ...
            else if(next_states[i].location >= 0){
                start_indexes[i] = env->curr_states.at(i).location;
                goal_indexes[i] = next_states[i].location;
            }
        }
        ...
        Instance instance = Instance("./example_problems/random.domain/maps/random-32-32-20.map", start_indexes, goal_indexes);
        Deadline deadline = Deadline(450);
        Solution solution = solve(instance, 1, &deadline, 123);

        for (int id : ids){
            if (next_states[id].location==-1){
                next_states.at(id) = State(solution[1][id]->index,-1,-1);
            }
        }
        ...
        //     // if agent is newly assigned a targeted next location, record the decision as not done yet
            if (next_states.at(id).location >=0){
                ...
                decided.at(id) = DCR({solution[1][id]->index,DONE::NOT_DONE});
            }
