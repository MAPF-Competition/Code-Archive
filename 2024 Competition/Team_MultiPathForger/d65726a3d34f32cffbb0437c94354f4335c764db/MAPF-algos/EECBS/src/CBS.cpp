...
/*void CBS::copyConflicts(const list<shared_ptr<EECBS_PREFIX_Conflict>>& conflicts,
	list<shared_ptr<EECBS_PREFIX_Conflict>>& copy, int excluded_agent) const
{
...
void CBS::copyConflicts(const list<shared_ptr<EECBS_PREFIX_Conflict >>& conflicts,
	list<shared_ptr<EECBS_PREFIX_Conflict>>& copy, const list<int>& excluded_agents)
{
...
void CBS::findConflicts(HLNode& curr, int a1, int a2)
{
	...
	for (int timestep = 0; timestep < min_path_length; timestep++)
	{
		...
		if (loc1 == loc2)
		{
			shared_ptr<EECBS_PREFIX_Conflict> conflict(new EECBS_PREFIX_Conflict());
...
shared_ptr<EECBS_PREFIX_Conflict> CBS::chooseConflict(const HLNode &node) const
{
	if (screen == 3)
		printConflicts(node);
	shared_ptr<EECBS_PREFIX_Conflict> choose;
...
void CBS::computeSecondPriorityForConflict(EECBS_PREFIX_Conflict& conflict, const HLNode& node)
{
	int count[2] = {0, 0};
...
void CBS::computeConflictPriority(shared_ptr<EECBS_PREFIX_Conflict>& con, CBSNode& node)
{
...
void CBS::classifyConflicts(CBSNode &node)
{
	...
	{
		shared_ptr<EECBS_PREFIX_Conflict> con = node.unknownConf.front();
		int a1 = con->a1, a2 = con->a2;
		int timestep = get<3>(con->constraint1.back());
		constraint_type type = get<4>(con->constraint1.back());
		...
		if (con->priority == conflict_priority::CARDINAL && heuristic_helper.type == EECBS_PREFIX_heuristics_type::ZERO)
		{
...
void CBS::removeLowPriorityConflicts(list<shared_ptr<EECBS_PREFIX_Conflict>>& conflicts) const
{
	if (conflicts.empty())
		return;
	unordered_map<int, shared_ptr<EECBS_PREFIX_Conflict> > keep;
	list<shared_ptr<EECBS_PREFIX_Conflict>> to_delete;
	for (const auto& conflict : conflicts)
	{
...
bool CBS::findPathForSingleAgent(CBSNode*  node, int ag, int lowerbound)
{
	...
	// find a path
	EECBS_PREFIX_Path new_path = search_engines[ag]->findOptimalPath(*node, initial_constraints[ag], paths, ag, lowerbound);
...
set<int> CBS::getInvalidAgents(const list<EECBS_PREFIX_Constraint>& constraints)  // return agents that violate the constraints
{
	set<int> agents;
	int agent, x, y, t;
	constraint_type type;
...
void CBS::saveStats(const string &fileName, const string &instanceName)
{
	...
	/*if (solution_found)
	{
		...
		int greedy = 0;
		heuristic_helper.type = EECBS_PREFIX_heuristics_type::DG;
...
string CBS::getSolverName() const
{
	...
	{
	case EECBS_PREFIX_heuristics_type::ZERO:
		if (PC)
			name += "ICBS";
		else
			name += "CBS";
		break;
	case EECBS_PREFIX_heuristics_type::CG:
		name += "CG";
		break;
	case EECBS_PREFIX_heuristics_type::DG:
		name += "DG";
		break;
	case EECBS_PREFIX_heuristics_type::WDG:
		name += "WDG";
		break;
	default:
		break;
...
CBS::CBS(vector<SingleAgentSolver*>& search_engines,
	...
	vector<EECBS_PREFIX_Path>& paths_found_initially, int screen) :
...
CBS::CBS(const EECBS_PREFIX_Instance& instance, bool sipp, int screen) :
	screen(screen), suboptimality(1),
