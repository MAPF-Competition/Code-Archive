...
void PlannerLong::plan(vector<Action> & actions, double timeLeft) {
    ...
    std::vector<int> actualDistance(m_num_of_agents, 0); // actualDistance[a] = length of the shortest path to its task
    std::vector<bool> doable(m_num_of_agents, 0); // doable[a] = true if there remains enough step to finish its task
    int num_undoable = 0; // number of agents that will not finish their task
    ...
    int num_reach = 0; // Number of agents whose planned path finishes the task
    int num_valid = 0; // Number of agents with a (valid) planned path
...
std::vector<int> PlannerLong::plan_path_with_conflicts(int agent, const Position &start, int end, int path_bound, std::vector<int> &enqueued, bool doable, std::vector<int> &actualDistance) {
    ...
    std::priority_queue<std::tuple<float,float,float,int,AstarNC*>, vector<std::tuple<float,float,float,int,AstarNC*>>> open_list;
    ...
    while (!open_list.empty()) {
        ...
        if(curr->g2 < path_bound) {
            auto neighborhood = neighbors_with_conflicts_v3(agent, Position(curr->location, curr->direction, curr->g2 + m_timestep));
            for (const auto &[neighbor,conflicts] : neighborhood) {
                ...
                if(next_node != nullptr) {
                    ...
                    // if(next_node->g2 > 20) {
                    //     next_node->conflict_cost = curr->conflict_cost;
                    // } else
                    {
                    ...
                    open_list.push(std::make_tuple(-next_node->conflict_cost,
                                                   -next_node->f,
                                                   -next_node->g,
                                                   2*(next_node->location == curr->location) + (next_node->direction == curr->direction),
                                                   next_node));
                }
            }
        }
...
std::vector<std::pair<Planner::Position,std::vector<int>>> PlannerLong::neighbors_with_conflicts_v3(int agent, const Planner::Position &position) const {
    std::vector<std::pair<Planner::Position,std::vector<int>>> neighbors;
    ...
    if (fw_loc >= 0 && !obstacle(fw_loc)) {   // Conflicts when movin forward
        std::unordered_set<int> conf_fw_set;

        if(m_ending_hash.contains(fw_loc)) {
            for(int ca : m_ending_hash.at(fw_loc)) {
                if (ca != agent && m_paths[ca].back().time <= t+1 && m_paths[ca].back().time + m_must_have_moved >= t + 1 && m_paths[ca].back().location == fw_loc)
                    conf_fw_set.insert(ca);
            }
        }

        if(m_passing_hash.contains(std::make_pair(fw_loc, t+1))) {
            for(int ca : m_passing_hash.at(std::make_pair(fw_loc, t+1))) {
                if (ca != agent ) {
                    assert(m_paths.at(ca).size() > t+1-m_timestep && m_paths.at(ca).at(t+1-m_timestep).location == fw_loc);
                    conf_fw_set.insert(ca);
                }
            }
        }
        ...
        // Edge conflicts
        if(m_passing_hash.contains(std::make_pair(fw_loc, t))) {
            for(int ca : m_passing_hash.at(std::make_pair(fw_loc, t))) {
                if(ca != agent && m_paths.at(ca).size() > t + 1 - m_timestep && m_paths.at(ca).at(t + 1 - m_timestep).location == position.location) {
        ...
        std::vector<int> conf_fw(conf_fw_set.begin(), conf_fw_set.end());
        neighbors.push_back(std::make_pair(move(position, Action::FW), conf_fw));
        // cout << " " << conf_fw.size();
    }

    { // Conflicts when staying in the same location
        std::unordered_set<int> conf_stay_set;
        int stay_loc = position.location;
        if(m_ending_hash.contains(stay_loc)) {
            for(int ca : m_ending_hash.at(stay_loc)) {
                if (ca != agent && m_paths[ca].back().time <= t+1 && m_paths[ca].back().time + m_must_have_moved >= t + 1 && m_paths[ca].back().location == stay_loc)
                    conf_stay_set.insert(ca);
            }
        }
        if(m_passing_hash.contains(std::make_pair(stay_loc, t+1))) {
            for(int ca : m_passing_hash.at(std::make_pair(stay_loc, t+1))) {
                if (ca != agent) {
        ...
        std::vector<int> conf_stay(conf_stay_set.begin(), conf_stay_set.end());
        for(int i = 0; i < 3; i++)
            neighbors.push_back(std::make_pair(stay_positions[i], conf_stay));
        // cout << " " << conf_stay.size() << " ";
    }

    // cout << endl;

    // auto test = neighbors_with_conflicts(agent, position);
    //
    // for(auto &[pos,conf] : neighbors) {
    //     for(auto &[postest,conftest] : test) {
    //         if(pos == postest && conf.size() != conftest.size()) {
    //             cout << conf.size() << " " << conftest.size() << endl;
    //             pos.print();
    //             postest.print();
    //             if(conf.size() == 1 && conftest.size() == 0 ) {
    //                 cout << "a" << agent << " conflicts with a" << conf[0] << endl;
    //                 for(auto &pos : m_paths.at(agent))
    //                     pos.print();
    //                 cout << "a" << conf[0] << endl;
    //                 for(auto &pos : m_paths.at(conf[0]))
    //                     pos.print();
    //                 _exit(1);
    //             }
    //         }
...
/// @param position Its current position
/// @return Vector of pairs of valid positions with a conflicting agent or -1 for no conflict
std::vector<std::pair<Planner::Position,std::vector<int>>> PlannerLong::neighbors_with_conflicts_v2(int agent, const Planner::Position &position) const {
    std::vector<std::pair<Planner::Position, std::vector<int>>> neighbors;
    const Position next_fw = move(position, Action::FW);
    if (next_fw.location >= 0 && !obstacle(next_fw.location)) {
...
/// @note We can use exact distances
