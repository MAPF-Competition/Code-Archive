...
LACAM_PREFIX_PIBT::LACAM_PREFIX_PIBT(const LACAM_PREFIX_Instance *_ins, LACAM_PREFIX_DistTable *_D, int seed, bool _flg_swap,
           Scatter *_scatter)
    : ins(_ins),
      MT(std::mt19937(seed)),
      N(ins->N),
      ...
      C_next(N, std::array<LACAM_PREFIX_Vertex *, 5>()),
...
LACAM_PREFIX_PIBT::~LACAM_PREFIX_PIBT() {}

bool LACAM_PREFIX_PIBT::set_new_config(const LACAM_PREFIX_Config &Q_from, LACAM_PREFIX_Config &Q_to,
...
bool LACAM_PREFIX_PIBT::funcPIBT(const int i, const LACAM_PREFIX_Config &Q_from, LACAM_PREFIX_Config &Q_to)
{
  const auto K = Q_from[i]->neighbor.size();

  // exploit scatter data
  LACAM_PREFIX_Vertex *prioritized_vertex = nullptr;
  if (scatter != nullptr) {
  ...
  std::sort(C_next[i].begin(), C_next[i].begin() + K + 1,
            [&](LACAM_PREFIX_Vertex *const v, LACAM_PREFIX_Vertex *const u) {
...
int LACAM_PREFIX_PIBT::is_swap_required_and_possible(const int i, const LACAM_PREFIX_Config &Q_from,
...
bool LACAM_PREFIX_PIBT::is_swap_required(const int pusher, const int puller,
                            LACAM_PREFIX_Vertex *v_pusher_origin, LACAM_PREFIX_Vertex *v_puller_origin)
{
  auto v_pusher = v_pusher_origin;
  auto v_puller = v_puller_origin;
  LACAM_PREFIX_Vertex *tmp = nullptr;
...
bool LACAM_PREFIX_PIBT::is_swap_possible(LACAM_PREFIX_Vertex *v_pusher_origin, LACAM_PREFIX_Vertex *v_puller_origin)
{
  // simulate pull
  auto v_pusher = v_pusher_origin;
  auto v_puller = v_puller_origin;
