...
namespace ZainPlanner
{
    ...
    const int maximum_number_of_planned_agents = 400;
    bool is_start;
    std::set<int> assigned_agents;
    ...
    void getNewInputs(vector<int> &free_agents, vector<int> &free_tasks, SharedEnvironment *env)
    {
        ...
        for(auto agent:free_agents){
            q.push({0, free_cells[env->curr_states[agent].location]*4+env->curr_states[agent].orientation});
        }
        while(!q.empty()){
            auto tmp = q.front();
            q.pop();
        ...
        for(auto agent:agents){
            ...
            if(dist[free_cells[task_location]] < cost){
                free_agents.push_back(agent);
                free_tasks.push_back(task);
            }
        }
        // for(int i=0; i<free_agents.size(); ++i){
        //     auto agent1 = free_agents[i];
        //     auto it = agents.begin(), it2 = agents.begin();
        //     while(it != agents.end()){
        //         int task_location = env->task_pool[env->curr_task_schedule[*it]].get_next_loc();
        //         int cost1 = realCost(env->curr_states[agent1].location, task_location);
        //         int cost2 = realCost(env->curr_states[*it].location, task_location);
        //         if(cost1 < cost2){
        //             free_agents.push_back(*it);
        //             free_tasks.push_back(env->curr_task_schedule[*it]);
        //             it2 = it;
        //             ++it;
        //             agents.erase(it2);
        //         }
        ...
        // for (auto task : env->task_pool)
        // {
        //     if (task.second.agent_assigned != -1 && task.second.idx_next_loc == 0)
        //     {
        //         if (near_agents[task.second.get_next_loc()])
        //         {
        //             free_tasks.push_back(task.first);
        //             free_agents.push_back(env->task_pool[task.first].agent_assigned);
        //         }
        //     }
        // }
        // if (env->curr_timestep % 50 == 0)
        // {
        //     int limit_number_agents = 100;
        //     if (map_is_random)
        //     {
        //         limit_number_agents = 1000;
        //     }
        //     else
        //     {
        //         limit_number_agents = 200; // min(200, 2*(int)free_agents.size());
        //     }
        //     vector<pair<int, int>> v;
        //     if (free_agents.size() < limit_number_agents)
        //     {
        //         for (auto task : env->task_pool)
        //         {
        //             // if(free_agents.size()>100)break;
        //             if (task.second.agent_assigned != -1 && task.second.idx_next_loc == 0)
        //             {
        //                 if (!near_agents[task.second.get_next_loc()])
        //                 {
        //                     v.push_back(make_pair(-taskCost(task.second, env), task.first));
        //                 }
        //             }
        //         }
        //     }
        ...
        //         if (free_agents.size() > limit_number_agents)
        //             break;
        //         free_tasks.push_back(it.second);
        //         free_agents.push_back(env->task_pool[it.second].agent_assigned);
        //     }
        // }
    ...
    void schedule_plan(int time_limit, std::vector<int> &proposed_schedule, SharedEnvironment *env)
    {
        ...
        for (auto task : free_tasks)
        {
            ...
            int new_cost = ZainPlanner::taskCost(env->task_pool[task]);
        ...
        for (auto location_and_vector_of_id_cost_task : location_to_task)
        {
            ...
            {
                free_tasks_locations_and_costs.push_back(make_pair(location_and_vector_of_id_cost_task.first, location_to_task[location_and_vector_of_id_cost_task.first][i].first - minimum_cost)); // {location, i-th minimum cost}
        ...
        ZainPlanner::dynInit(free_tasks_locations_and_costs.size());
        ZainPlanner::fillInputs(free_agents_location, free_tasks_locations_and_costs);
        ZainPlanner::dynGenAllNeighbors(free_tasks_locations_and_costs.size());
        // cout<<"-------"<<endl;
        // for(int i=0; i<env->num_of_agents; ++i){
        //     cout<<"agent "<<i<<"at "<<env->curr_states[i].location<<" "<<proposed_schedule[i]<<endl;
        // }
        // cout<<"tasks"<<endl;
        // for(auto it:location_to_task){
        //     for(auto it1:it.second)
        //     cout<<"task "<<it1.second<<" at location "<<it.first<<endl;
        // }
        // ZainPlanner::solve(maximum_number_of_planned_agents-(env->num_of_agents-(int)free_agents_location.size()));
        ZainPlanner::solve(maximum_number_of_planned_agents);
        vector<int> new_assignments; // new_assignment: map_size -> map_size. It gives the assignment of the start location to the goal location.
        ZainPlanner::getAssignments(new_assignments);
        ZainPlanner::dynReset();
        ...
        for (int i = 0; i < free_agents.size(); ++i)
        {
            ...
            {
                if(is_start){
                    if(assigned_agents.size() < maximum_number_of_planned_agents){
                        assigned_agents.insert(i);
                ...
                // assert(location_to_task.count(new_assignments[free_agents_location[i]]));
                    proposed_schedule[free_agents[i]] = location_to_task[new_assignments[free_agents_location[i]]].back().second;
                    location_to_task[new_assignments[free_agents_location[i]]].pop_back();
                }
            }
    ...
    {
        cell1 = free_cells[cell1];
        cell2 = free_cells[cell2];
        if (cell1 > cell2)
    ...
    void calculateRealDistances(SharedEnvironment *env)
    {
        ...
        for (long long cell1 = 0; cell1 < number_of_free_cells; ++cell1)
        {
            ...
            while (!q.empty())
            {
                ...
                int cell = inverse_free_cells[cell2];
                int x = cell / env->cols, y = cell % env->cols;
                int xx = x + dx[dir], yy = y + dy[dir];
                int id = xx * env->cols + yy;
                if (xx < env->rows && xx >= 0 && yy < env->cols && yy >= 0)
        ...
        // for(long long cell1 = 0; cell1 < number_of_free_cells; ++cell1){
        //     for(long long i = cell1; i<number_of_free_cells; ++i){
        //         if(cell1*number_of_free_cells-(cell1*(cell1-1)/2)+i-cell1 != cnt){
        //             cout<<cell1<<" "<<i<<" "<<cell1*number_of_free_cells-(cell1*(cell1-1)/2)+i-cell1<<" "<<cnt<<endl;
        //             exit(0);
        //         }
        //         if(distancesBetweenCells[cell1*number_of_free_cells-(cell1*(cell1-1)/2)+i-cell1]==-1){
        //             cout<<-1<<endl;
        //             exit(0);
        //         }
        //         ++cnt;
        ...
        else
        {
            ...
            for (long long cell1 = 0; cell1 < number_of_free_cells; ++cell1)
            {
                for (long long cell2 = cell1; cell2 < number_of_free_cells; ++cell2)
                {
                    ...
                    // in>>distancesBetweenCells[cell1*number_of_free_cells+cell2];
                    // distancesBetweenCells[cell2*number_of_free_cells+cell1] = distancesBetweenCells[cell1*number_of_free_cells+cell2];
                }
            }
    ...
    void schedule_initialize(int preprocess_time_limit, SharedEnvironment *env)
    {
        ...
        ZainPlanner::readMap(env);
        ZainPlanner::compressFreeCells();
        ZainPlanner::genAllNeighbors();
        if (map_is_random)
            storeUndesiredLocations(env);
        // readDistances(env);
