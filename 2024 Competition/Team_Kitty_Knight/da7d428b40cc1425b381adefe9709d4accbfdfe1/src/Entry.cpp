...
void Entry::compute(int time_limit, std::vector<Action> & plan, std::vector<int> & proposed_schedule)
{
    ...
    // g_timer.record_d("_step_s","_step_scheduler_e","_step_scheduler");
    ...
    // g_timer.record_d("_step_scheduler_e","_step_planner_e","_step_planner");

    // g_timer.record_d("_step_s","_step_e","_step");
...
void Entry::update_goal_locations(std::vector<int> & proposed_schedule)
{
    ...
    for (size_t i = 0; i < proposed_schedule.size(); i++)
    {
        ...
        if (t_id == -1) {
            // TODO: maybe we could handle it better, e.g. perhaps move to less congested region or where tasks appear more often.
            // ideally, this should be fixed in planner!
