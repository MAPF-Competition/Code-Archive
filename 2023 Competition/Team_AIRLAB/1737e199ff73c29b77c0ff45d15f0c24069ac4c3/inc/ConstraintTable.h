...
class ConstraintTable {
 public:
  ...
  ConstraintTable(size_t num_col, size_t map_size, const PathTable* path_table_for_CT = nullptr,
      ...
      : num_col(num_col),
        map_size(map_size),
        path_table_for_CT(path_table_for_CT),
  ...
  void insert2CT(size_t loc, int t_min,
                 int t_max);  // insert a vertex constraint to the constraint table
  void insert2CT(size_t from, size_t to, int t_min,
                 int t_max);  // insert an edge constraint to the constraint table
  void insert2CAT(
      int agent,
      const vector<Path*>& paths);    // build the conflict avoidance table using a set of paths
 ...
 protected:
  ...
  map<int, size_t> landmarks;  // <timestep, location>: the agent must be at the given location at
                               // the given timestep

  void insertLandmark(size_t loc, int t);
