...
{
    cout << "Use collision version of A*" << endl;
    agentLocationMap = env->map;
    actions = std::vector<Action>(env->curr_states.size(), Action::W);
    for (int i = 0; i < env->num_of_agents; i++) 
    {
        agentLocationMap[env->curr_states[i].location] = 1;
    }
    for (int i = 0; i < env->num_of_agents; i++) 
    {
        agentLocationMap[env->curr_states[i].location] = 0;
        ...
        {
            path = single_agent_plan_collision(env->curr_states[i].location,
                                    env->curr_states[i].orientation,
                                    env->goal_locations[i].front().first);
        }
        agentLocationMap[path.front().first] = 1;
        if (DEBUG == true)
        {
            if (i == 19 || i == 8){
                cout << "agent id:" << i <<endl;
                cout << "agent location:" << env->curr_states[i].location<<endl;
                cout << "agent orientation:" << env->curr_states[i].orientation<<endl;
                cout << "agent first move:" << path.front().first<<endl;
...
list<pair<int,int>> MAPFPlanner::single_agent_plan_collision(int start,int start_direct,int end)
{
    list<pair<int,int>> path;
    ...
    while (!open_list.empty())
    {
        ...
        list<pair<int,int>> neighbors = getNeighborsCollision(curr->location, curr->direction);
...
bool MAPFPlanner::validateMoveCollision(int loc, int loc2)
{
    int loc_x = loc/env->cols;
    int loc_y = loc%env->cols;

    if (loc_x >= env->rows || loc_y >= env->cols || env->map[loc] == 1 || agentLocationMap[loc] == 1)
        return false;
...
list<pair<int,int>> MAPFPlanner::getNeighborsCollision(int location,int direction)
{
    list<pair<int,int>> neighbors;
    //forward
    int candidates[4] = { location + 1,location + env->cols, location - 1, location - env->cols};
    int forward = candidates[direction];
    int new_direction = direction;
    if (forward>=0 && forward < env->map.size() && validateMoveCollision(forward,location))
        neighbors.emplace_back(make_pair(forward,new_direction));
    //turn left
    new_direction = direction-1;
    if (new_direction == -1)
