...
namespace GameScheduler
{
    ...
    int maximum_number_of_planned_agents = 2200;
    bool is_start;
    ...
    void getNewInputs(vector<int> &free_agents, vector<int> &free_tasks, SharedEnvironment *env)
    {
        ...
        if(is_start){
            ...
            assert(assigned_agents.size()==maximum_number_of_planned_agents);
        ...
        for(auto agent:agents){
            ...
            if(dist[AMAPFDSOCPlanner::free_cells[task_location]] < cost){
    ...
    void schedule_plan(int time_limit, std::vector<int> &proposed_schedule, SharedEnvironment *env)
    {
        ...
        for (auto task : free_tasks)
        {
            ...
            int new_cost = GameScheduler::taskCost(env->task_pool[task]);
        ...
        // if(env->curr_timestep == 50){
        //     maximum_number_of_planned_agents += 500;
        //     is_start = true;
        // }
        ...
        //     is_start = true;
        // }
        if(free_tasks_locations_and_costs.size() < 1.2 * min(maximum_number_of_planned_agents, (int)free_agents_location.size())){
        ...
        int max_number_of_assignments =  min((int)(free_tasks_locations_and_costs.size()), (int)(free_agents_location.size()));
        max_number_of_assignments = min(max_number_of_assignments, maximum_number_of_planned_agents);
            ...
            else
            {
                if(is_start){
                    if(assigned_agents.size() < maximum_number_of_planned_agents){
                        assigned_agents.insert(free_agents[i]);
