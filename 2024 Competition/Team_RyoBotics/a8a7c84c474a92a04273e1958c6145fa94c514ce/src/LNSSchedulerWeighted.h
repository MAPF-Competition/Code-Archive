...
namespace LNSSchedulerWeighted
{
    ...
    std::pair<WeightedCostType, std::unordered_set<int>> evaluateSchedule(const ScheduleContainer &schedule, SharedEnvironment *env, WeightedCostType lower_bound = std::numeric_limits<WeightedCostType>::max())
    {
        ...
        if constexpr (std::is_same_v<ScheduleContainer, std::vector<int>>)
        {
            for (int agent_id : SchedulerUtils::global_available_agents)
            {
                ...
                WeightedCostType completion_time = calculateTaskCompletionTime(agent_id, schedule[agent_id], env);
        ...
        else
        {
            for (const auto &[agent_id, task_id] : schedule)
            {
                ...
                WeightedCostType completion_time = calculateTaskCompletionTime(agent_id, task_id, env);
                if (include_additional_cost_in_evaluation)
                {
    ...
    // スケジュールの評価用関数をテンプレート化
    template <typename ScheduleContainer>
    std::pair<WeightedCostType, std::unordered_set<int>> evaluateScheduleSoC(const ScheduleContainer &schedule, SharedEnvironment *env, WeightedCostType lower_bound = std::numeric_limits<WeightedCostType>::max())
    {
        WeightedCostType makespan = 0;
        std::unordered_set<int> critical_agents;
        WeightedCostType soc = 0;
