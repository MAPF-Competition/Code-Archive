...
namespace LNSSchedulerWeighted
{
    ...
    bool INCLUDE_FLOW_COST = false;
    // スレッド数を制御するための定数を追加
    const int NUM_THREADS = std::thread::hardware_concurrency(); // 利用可能なCPUコア数
    const int TASKS_PER_THREAD = 400;                            // 1スレッドあたりの処理タスク数
    int assinged_task_additional_cost = 2;
    // 　今のタスクと同一の場合差し引く値
    int same_task_additional_cost = -2;
    // 同時にタスクを割り当て可能なagent数
    int MAX_ASSIGNED_TASKS = INT_MAX;
    int MAX_FAILED_ATTEMPTS_lns = 100;
    const double NEARBY_AGENTS_RATIO = 0.65;
    bool count_deadends = false;
    // 距離の閾値を計算するための除数
    const int DISTANCE_THRESHOLD_DIVISOR = 3;
    const int TASK_SWITCH_THRESHOLD = 3;
    int estimated_sim_time = INT_MAX;
    bool disable_agents = false;
    bool include_additional_cost_in_evaluation = true;
    ...
    void schedule_initialize(int preprocess_time_limit, SharedEnvironment *env)
    {
        ...
        MANHATTAN_DISTANCE_THRESHOLD = std::max(env->rows, env->cols) / DISTANCE_THRESHOLD_DIVISOR;
        if (env->map_name == "sortation_large.map" || env->map_name == "warehouse_large.map")
        {
            include_waypoints = false;
            estimated_sim_time = 5000;
            ...
            {
                assinged_task_additional_cost = 4;
                same_task_additional_cost = -4;
            }
        }
        else if (env->map_name == "Paris_1_256.map")
        {
            ...
            estimated_sim_time = 3000;
        }
        else if (env->map_name == "random-32-32-20.map" && env->num_of_agents <= 700)
        {
            assinged_task_additional_cost = 3;
            same_task_additional_cost = -3;
            // MAX_ASSIGNED_TASKS = 50;
            if (env->num_of_agents <= 200)
            {
                estimated_sim_time = 600;
            }
            else if (env->num_of_agents <= 600)
            {
                estimated_sim_time = 900;
            }
            else
            {
                estimated_sim_time = 2000;
            }
        }
        else if (env->map_name == "brc202d.map")
        {
            assinged_task_additional_cost = 10;
            same_task_additional_cost = -10;
            disable_agents = true;
            MAX_ASSIGNED_TASKS = 2400;
            estimated_sim_time = 5000;
        }
        ...
        {
            SchedulerUtils::trajLNS_copy.env = env;
        }
        // same_task_additional_cost = 0;
        // assinged_task_additional_cost = 0;
    }
    // タスクの完了時間を計算
    WeightedCostType calculateTaskCompletionTime(int agent_id, int task_id, SharedEnvironment *env)
    {
        if (task_id == -1)
        {
        ...
        // 最初の位置までのコスト
        total_cost = SchedulerUtils::getWeightedCostFromStateToLocation(current_loc, current_dir, task.locations[0]);
        ...
        // if (SchedulerUtils::include_tabu_cost)
        // {
        //     total_cost += SchedulerUtils::getVisitedTabuStates(current_loc, current_dir, task.locations[0], 0) * SchedulerUtils::tabu_cost_multiplier;
        // }
    ...
    {
        std::vector<int> free_agents(SchedulerUtils::scheduling_target_agents.begin(), SchedulerUtils::scheduling_target_agents.end());
        ...
        // nearby_agentsから選ぶ割合を定義
        int nearby_agents_needed = static_cast<int>(additional_needed * NEARBY_AGENTS_RATIO);
        ...
        std::vector<int> tmp_nearby_agents(nearby_agents.begin(), nearby_agents.end());
        std::shuffle(tmp_nearby_agents.begin(), tmp_nearby_agents.end(), rng);
        // nearby_agentsがnum_destroyを超える場合は、nearby_agentsをシャッフルして追加
        // isOverTaskSwitchesを考慮
        std::vector<int> remaining_agents;
        std::unordered_set<int> selected_nearby_agents;
        for (int agent : tmp_nearby_agents)
        {
            if (remaining_agents.size() >= nearby_agents_needed)
            {
                break;
            }
            if (!isOverTaskSwitches(agent))
            {
                ...
                selected_nearby_agents.insert(agent);
            }
        }
        if (remaining_agents.size() >= additional_needed)
        {
            std::cerr << "!! remaining_agents.size(): " << remaining_agents.size() << std::endl;
        ...
        else
        {
            ...
            while (non_critical_agents.size() < additional_needed && free_agents.size() > 0)
            {
                ...
                if (critical_agents.find(agent) == critical_agents.end() &&
                    selected_nearby_agents.find(agent) == selected_nearby_agents.end() &&
        ...
        remaining_agents.erase(remaining_agents.begin() + std::min(additional_needed,
                                                                   static_cast<int>(remaining_agents.size())),
                               remaining_agents.end());
        // result.insert(result.end(),
        ...
        {
            // すでに割当られているagentの数
            int already_assigned_agents = 0;
            std::vector<int> tmp_must_assign_agents;
            std::vector<int> other_agents;
            for (int agent_id : remaining_agents)
            {
                if (schedule[agent_id] != -1)
                {
                    already_assigned_agents++;
                }
                if (SchedulerUtils::must_assign_agents.find(agent_id) != SchedulerUtils::must_assign_agents.end())
                {
                    tmp_must_assign_agents.push_back(agent_id);
                }
                else
                {
                    other_agents.push_back(agent_id);
                }
            }
            if (tmp_must_assign_agents.size() == 0)
            {
                result.insert(result.end(),
                              remaining_agents.begin(),
                              remaining_agents.end());
            ...
            int must_assign_agents_needed = std::max(static_cast<int>(tmp_must_assign_agents.size()), already_assigned_agents);
            remaining_agents.insert(remaining_agents.end(),
                                    tmp_must_assign_agents.begin(),
                                    tmp_must_assign_agents.end());
            remaining_agents.insert(remaining_agents.end(),
                                    other_agents.begin(),
                                    other_agents.begin() + must_assign_agents_needed - tmp_must_assign_agents.size());
            std::shuffle(remaining_agents.begin(), remaining_agents.end(), rng);
            remaining_agents.insert(remaining_agents.end(),
                                    other_agents.begin() + must_assign_agents_needed - tmp_must_assign_agents.size(),
                                    other_agents.end());

            result.insert(result.end(), remaining_agents.begin(), remaining_agents.end());
        }
        else
        {
            result.insert(result.end(),
                ...
                std::unordered_map<unsigned short, std::vector<TaskCost>> &agent_task_costs)
    {
        ...
        for (int agent_id : destroyed_agents)
        {
            ...
            else
            {
                ...
                {
                    WeightedCostType cost = calculateTaskCompletionTime(agent_id, task_id, env);
                ...
                agent_task_costs.emplace(agent_id, task_costs);
            }
    ...
    void optimizeAssignment(std::vector<int> &proposed_schedule, InitialScheduleResult &initial_result, SharedEnvironment *env, std::chrono::steady_clock::time_point end_time)
    {
        ...
        {
            std::vector<int> tmp_nearby_agents = getNearbyAgents(agent_id, MANHATTAN_DISTANCE_THRESHOLD, critical_agents, env);
        ...
        while (std::chrono::steady_clock::now() < end_time && iteration < max_iterations && failed_attempts < MAX_FAILED_ATTEMPTS_lns)
        {
            // 破壊する比率を徐々に減少させる
            ...
            std::vector<int> destroyed_agents = destroy(current_destroy_ratio, critical_agents, nearby_agents, best_schedule);
    ...
    InitialScheduleResult generateInitialSchedule(std::vector<int> &proposed_schedule,
                                                  ...
                                                  std::chrono::steady_clock::time_point end_time)
    {
        const std::unordered_set<unsigned short> &scheduling_target_agents = SchedulerUtils::scheduling_target_agents;
        const std::unordered_set<int> &scheduling_target_tasks = SchedulerUtils::scheduling_target_tasks;
        std::unordered_set<int> assigned_tasks;
        WeightedCostType makespan = 0;
        std::unordered_set<int> critical_agents;
        std::unordered_map<unsigned short, std::vector<TaskCost>> agent_task_costs;
        std::unordered_map<int, WeightedCostType> agent_completion_times_local;
        int count = SchedulerUtils::current_task_assignments.size();
        // std::cout << "count as start of generateInitialSchedule: " << count << std::endl;
        if (disable_agents)
        {
            for (int agent_id : SchedulerUtils::must_assign_agents)
            {
                if (std::chrono::steady_clock::now() >= end_time)
                {
                    break;
                ...
                {
                    std::vector<TaskCost> task_costs = computeSortedTaskCostsForAgent(agent_id, env);
                    agent_task_costs.emplace(agent_id, std::move(task_costs));
                    bool found_task = false;
                    for (const TaskCost &task_cost : agent_task_costs[agent_id])
                    {
            ...
            int num_agents_to_disable = count - MAX_ASSIGNED_TASKS;
            if (num_agents_to_disable > 0)
            {
                // must_assign_agentsに含まれていないagentのリストを作成
                std::vector<int> candidates;
                for (int agent_id : scheduling_target_agents)
                {
                    if (SchedulerUtils::must_assign_agents.find(agent_id) == SchedulerUtils::must_assign_agents.end() &&
                        proposed_schedule[agent_id] != -1)
                    {
                        candidates.push_back(agent_id);
                    }
                }
                ...
                int disabled = 0;
                while (disabled < num_agents_to_disable && !candidates.empty())
                {
                    int agent_id = candidates.back();
                    candidates.pop_back();
                    proposed_schedule[agent_id] = -1;
                    disabled++;
                    count--;
        ...
        // std::cout << "count after disable_agents: " << count << std::endl;
        ...
        // std::cout << "count as end of generateInitialSchedule: " << count << std::endl;
        return {makespan, critical_agents, std::move(agent_task_costs)};
    }
    ...
    {
        return calculateTaskSwitchLevel(agent_id) > TASK_SWITCH_THRESHOLD;
    }
    ...
    std::vector<int> getNearbyAgents(int agent_id, int distance_threshold, const std::unordered_set<int> &critical_agents, SharedEnvironment *env)
    {
        ...
        for (int other_agent : SchedulerUtils::scheduling_target_agents)
        {
            if (other_agent == agent_id || critical_agents.find(other_agent) != critical_agents.end())
                continue;
            ...
            WeightedCostType distance = SchedulerUtils::getWeightedCostFromStateToLocation(agent_location, agent_dir, other_location);
    ...
    {
        // SchedulerUtils 内のターゲットタスク集合を取得
        const auto &scheduling_target_tasks = SchedulerUtils::scheduling_target_tasks;
        std::vector<TaskCost> task_costs;
        task_costs.reserve(scheduling_target_tasks.size());
        ...
        for (int task_id : scheduling_target_tasks)
        {
            if (count_deadends && SchedulerUtils::hasDeadEndInErrand(task_id, env))
            {
                continue;
            }
            ...
            futures.push_back(std::async(std::launch::async,
                                         [agent_id, batch, env]() -> std::vector<TaskCost>
                                         {
                                             ...
                                             for (int task_id : batch)
                                             {
                                                 ...
                                                 //      cost += assinged_task_additional_cost;
                                                 //  }
                                                 cost += calculateAdditionalCost(agent_id, task_id, env);
                                                 if (INCLUDE_FLOW_COST)
                                                 {
        ...
        for (auto &future : futures)
        {
            ...
            task_costs.insert(task_costs.end(), batch_results.begin(), batch_results.end());
        }
        std::sort(task_costs.begin(), task_costs.end(),
                  [](const TaskCost &a, const TaskCost &b)
                  {
    ...
    {
        CostType additional_cost = 0;
        int current_assigned_agent = env->task_pool[task_id].agent_assigned;
        if (current_assigned_agent != -1 && current_assigned_agent != agent_id)
        {
            // additional_cost += assinged_task_additional_cost * (calculateTaskSwitchLevel(current_assigned_agent) + 1);
            additional_cost += assinged_task_additional_cost;
        }
        else if (current_assigned_agent == agent_id)
        {
            additional_cost += same_task_additional_cost;
        }
                                      ...
                                      std::chrono::steady_clock::time_point end_time)
    {
        ...
        if (disable_agents)
        {
            for (int agent_id : SchedulerUtils::must_assign_agents)
            {
                ...
                if (env->curr_task_schedule[agent_id] == -1)
                {
                    MinTaskCost min_task_cost = findMinCostTask(agent_id, scheduling_target_tasks, assigned_tasks, env, end_time, true);
                    if (min_task_cost.found)
                    {
                        proposed_schedule[agent_id] = min_task_cost.task_id;
                        assigned_tasks.insert(min_task_cost.task_id);
                        count++;
                    }
                }
            }
        ...
        for (int agent_id : scheduling_target_agents)
        {
            ...
            // if (min_task_cost.cost > makespan)
            // {
            //     makespan = min_task_cost.cost;
            //     critical_agents.clear();
            //     critical_agents.insert(agent_id);
            // }
            // else if (min_task_cost.cost == makespan)
            // {
            //     critical_agents.insert(agent_id);
            // }
        }
    ...
    //                             SharedEnvironment *env,
    //                             std::chrono::steady_clock::time_point end_time,
    //                             bool ignore_assigned)
    // {
    //     int min_cost = INT_MAX;
    //     int min_task_id = -1;
    //     bool found = false;

    //     int is_assigned_agent = env->curr_task_schedule[agent_id] != -1;
    ...
    //         // ignore_assignedがtrueの場合のみ、アサイン済みタスクをスキップ
    //         if (ignore_assigned)
    //         {
    //             int is_assigned_task = env->task_pool[task_id].agent_assigned != -1 &&
    //                                    env->task_pool[task_id].agent_assigned != agent_id;
    //             if (is_assigned_task)
    //             {
    //                 continue;
    //             }
    ...
    //     return {min_task_id, min_cost, found};
    // }
                                ...
                                bool ignore_assigned)
    {
        ...
        std::vector<TaskCost> task_costs = computeTaskCostsForAgent(agent_id, env, assigned_tasks, ignore_assigned);
        ...
        {
            int task_id = task_cost.task_id;
            if (std::chrono::steady_clock::now() >= end_time)
            {
                break;
            }

            if (task_cost.cost < min_cost)
            {
                min_cost = task_cost.cost;
                min_task_id = task_id;
                found = true;
            }
        }
    ...
    // 新しく追加する関数: agent_task_switches[agent_id] + 9) / 10 を計算する
    int calculateTaskSwitchLevel(int agent_id)
    {
        return (agent_task_switches[agent_id] + 9) / 10;
    }
