...
std::vector<HeuristicTable> global_heuristictable;		// the heuristic table for each goal location
Neighbors global_neighbors;								// the neighbor table for each location


/**
 * @brief Initialize the neighbor table
 */
void init_neighbor(SharedEnvironment* env){
	...
	for (int row=0; row<env->rows; row++){
		for (int col=0; col<env->cols; col++){
			...
			if (env->map[loc]==0){
				if (row>0 && env->map[loc-env->cols]==0){		// check the north neighbor
					global_neighbors[loc].push_back(loc-env->cols);
				}
				if (row<env->rows-1 && env->map[loc+env->cols]==0){ // check the south neighbor
					global_neighbors[loc].push_back(loc+env->cols);
				}
				if (col>0 && env->map[loc-1]==0){				// check the west neighbor
					global_neighbors[loc].push_back(loc-1);
				}
				if (col<env->cols-1 && env->map[loc+1]==0){		// check the east neighbor
					global_neighbors[loc].push_back(loc+1);
				}
			}
		}
...
/**
 * @brief initialize the distance to path table
 */
...
/**
 * @brief get the <deviation,> pair source(current location) to path distance
 * @return std::pair<int,int> first is the distance to the path, second is the distance from the path to the goal
 */
