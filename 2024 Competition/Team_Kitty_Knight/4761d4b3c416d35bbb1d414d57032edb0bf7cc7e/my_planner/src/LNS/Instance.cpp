	...
	map_fname(map_fname), agent_fname(agent_fname), num_of_agents(num_of_agents)
{
	...
	if (!succ)
	{
		...
		{
			std::cerr << "Map file " << map_fname << " not found." << endl;
			exit(-1);
		}
	}
	...
	if (!succ)
	{
		...
		{
			std::cerr << "Agent file " << agent_fname << " not found." << endl;
			exit(-1);
		}
	}
...
bool Instance::loadAgents()
{
	...
	if (nathan_benchmark) // Nathan's benchmark
	{
		...
		for (int i = 0; i < num_of_agents; i++)
		{
			...
			if (line.empty())
            {
			    std::cerr << "Error! The instance has only " << i << " agents" << endl;
			    exit(-1);
            }
			tokenizer< char_separator<char> > tok(line, sep);
...
bool Instance::validateSolution(const vector<Path*>& paths, int sum_of_costs, int num_of_colliding_pairs) const
{
    ...
    {
        std::cerr << "We have " << paths.size() << " for " << start_locations.size() << " agents." << endl;
        exit(-1);
    }
    int sum = 0;
    for (auto i = 0; i < start_locations.size(); i++)
    {
        ...
        {
            std::cerr << "No path for agent " << i << endl;
            exit(-1);
        }
        else if (start_locations[i] != paths[i]->front().location)
        {
            std::cerr << "The path of agent " << i << " starts from location " << paths[i]->front().location
                 << ", which is different from its start location " << start_locations[i] << endl;
            exit(-1);
        }
        else if (goal_locations[i] != paths[i]->back().location)
        {
            std::cerr << "The path of agent " << i << " ends at location " << paths[i]->back().location
        ...
        for (int t = 1; t < (int) paths[i]->size(); t++ )
        {
            ...
            {
                std::cerr << "The path of agent " << i << " jumps from "
    ...
    if (sum_of_costs != sum)
    {
        std::cerr << "The computed sum of costs " << sum_of_costs <<
        ...
        for (auto j = i + 1; j < start_locations.size(); j++)
        {
            ...
            for (; t < (int) paths[a1]->size(); t++)
            {
                if (paths[a1]->at(t).location == paths[a2]->at(t).location) // vertex conflict
                {
                    ...
                    {
                        std::cerr << "Find a vertex conflict between agents " << a1 << " and " << a2 <<
                        ...
                        paths[a1]->at(t-1).location == paths[a2]->at(t).location) // edge conflict
                {
                    if (num_of_colliding_pairs == 0)
                    {
                        std::cerr << "Find an edge conflict between agents " << a1 << " and " << a2 <<
            ...
            if (!found_collision)
            {
                ...
                for (; t < (int) paths[a2]->size(); t++)
                {
                    if (paths[a2]->at(t).location == target)  // target conflict
                    {
                        if (num_of_colliding_pairs == 0)
                        {
                            std::cerr << "Find a target conflict where agent " << a2 << " (of length " << paths[a2]->size() - 1 <<
    ...
    {
        std::cerr << "The computed number of colliding pairs " << num_of_colliding_pairs <<
