...
bool PlannerGame::conflict_solver(int agent1, int time) {
    ...
    int old_time = m_time_to_move.at(agent1);
    if (reset_count)
    ...
    m_queue.push(agent1);
    m_enqueued.at(agent1) += 1;
    m_forward.at(agent1) = true;
    m_time_to_move.at(agent1) = 3; // @note I set it to the maximum now, it will be corrected if the planning is successful
    ...
    do {
        ...
        const int cur_loc = best_move(a, time, collisions);
        ...
        for (int a2 : collisions) {
            ...
            const int loc = m_location.at(a2).at(time + 1);
            assert(loc >= 0);
            if (get_agent(loc, time + 1) == a2)
                set_agent(loc, time + 1, -1);
            m_location.at(a2).at(time + 1) = -1;
            m_queue.push(a2);
    ...
    if (invalid) {
        ...
        m_forward.at(agent1) = false;
        m_time_to_move.at(agent1) = old_time;
...
std::vector<int> PlannerGame::get_collisions(int agent, int next_location, int time) const {
    std::vector<int> collisions;
    const int cur_location = m_location.at(agent).at(time);
    // vertex collision
    int a1 = get_agent(next_location, time + 1);
    if (a1 >= 0 && a1 != agent) {
        collisions.push_back(a1);
    }
    // edge collision
    int a2 = get_agent(next_location, time);
    if (a2 >= 0 && m_location.at(a2).at(time+1) == cur_location) {
...
int PlannerGame::best_move(int agent, int time, std::vector<int> & collisions) {
    int best_location = -1;
    double best_value = -1;
    collisions.clear();
    const std::list<int> locations = next_locations(agent, time);
    for (int loc : locations) {
        std::vector<int> cur_collisions = get_collisions(agent, loc, time);
    ...
    // Update data
    const int old_loc = m_location.at(agent).at(time + 1);
    if (old_loc >= 0) { // if the agent was planned
        set_agent(old_loc, time + 1, -1);
    }
    m_location.at(agent).at(time + 1) = best_location;
    set_agent(best_location, time + 1, agent);
...
std::list<int> PlannerGame::next_locations(int agent, int time) const {
    const int cur_loc = m_location.at(agent).at(time);
...
std::vector<int> PlannerGame::sorted_agents(int time) const {
    // Compute approximate distances
    std::vector<int> dist(env->num_of_agents, -1);
    for (int a = 0; a < env->num_of_agents; a++) {
        const int source = m_location.at(a).at(time);
