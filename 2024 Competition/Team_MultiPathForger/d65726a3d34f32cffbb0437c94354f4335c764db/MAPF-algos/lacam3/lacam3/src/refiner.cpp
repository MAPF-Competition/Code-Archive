...
LACAM_PREFIX_Solution refine(const LACAM_PREFIX_Instance *ins, const Deadline *deadline,
                const LACAM_PREFIX_Solution &solution, LACAM_PREFIX_DistTable *D, const int seed,
                const int verbose)
{
  if (solution.empty()) return LACAM_PREFIX_Solution();
  ...
  auto CT = LACAM_PREFIX_CollisionTable(ins);
  ...
  for (auto k = 0; (k + 1) * num_refine_agents < N; ++k) {
    if (is_expired(deadline)) return LACAM_PREFIX_Solution();
