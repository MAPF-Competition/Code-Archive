...
public:
    ...
    //EECBS_PREFIX_Path findOptimalPath(const PathTable& path_table) {return EECBS_PREFIX_Path(); } // TODO: To implement
    //EECBS_PREFIX_Path findOptimalPath(const ConstraintTable& constraint_table, const PathTableWC& path_table);
    EECBS_PREFIX_Path findOptimalPath(const HLNode& node, const ConstraintTable& initial_constraints,
                         const vector<EECBS_PREFIX_Path*>& paths, int agent, int lowerbound);
    pair<EECBS_PREFIX_Path, int> findSuboptimalPath(const HLNode& node, const ConstraintTable& initial_constraints,
                                       const vector<EECBS_PREFIX_Path*>& paths, int agent, int lowerbound, double w);  // return the path and the lowerbound
    EECBS_PREFIX_Path findPath(const ConstraintTable& constraint_table); // return A path that minimizes collisions, breaking ties by cost
    ...
    SIPP(const EECBS_PREFIX_Instance& instance, int agent):
...
private:
    ...
    // EECBS_PREFIX_Path findNoCollisionPath(const ConstraintTable& constraint_table);
