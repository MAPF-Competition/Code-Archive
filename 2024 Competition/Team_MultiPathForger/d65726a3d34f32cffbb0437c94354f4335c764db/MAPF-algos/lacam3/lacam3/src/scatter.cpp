...
Scatter::Scatter(const LACAM_PREFIX_Instance *_ins, LACAM_PREFIX_DistTable *_D, const Deadline *_deadline,
...
void Scatter::construct()
{
  ...
  using ScatterNode = std::tuple<LACAM_PREFIX_Vertex *, int, int, int, LACAM_PREFIX_Vertex *>;
  ...
  auto CLOSED = std::vector<LACAM_PREFIX_Vertex *>(V_size, nullptr);  // parent

  // metrics
  auto collision_cnt_last = 0;
  auto paths_prev = std::vector<LACAM_PREFIX_Path>();
