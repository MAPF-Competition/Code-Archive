...
RHCR::MAPFSolver *CUDAPlanner::rhcr_build_mapf_solver(nlohmann::json &config, RHCR::CompetitionGraph &graph)
{
...
void CUDAPlanner::rhcr_config_solver(std::shared_ptr<RHCR::RHCRSolver> &solver, nlohmann::json &config)
{
    ...
    if (solver->simulation_window > solver->planning_window)
    {
        cerr << boost::format("Error: the simulation window %d can not be larger than the planning window %d!") %
                    solver->simulation_window % solver->planning_window
             << endl;
        exit(1);
    }
...
std::string CUDAPlanner::load_map_weights(string weights_path)
{
    // : make weights float
...
void CUDAPlanner::initialize(int preprocess_time_limit)
{
    cout << "planner initialization begins" << endl;
    load_configs();

    if (env->num_of_agents <= 1500)
    {
        addition_time = std::max(5 * env->num_of_agents / 100, 35);
    }
    else
    {
        addition_time = 1 * env->num_of_agents / 100;
    }
...
void CUDAPlanner::plan(int time_limit, vector<Action> &actions)
{
    ...
    int limit = time_limit - std::chrono::duration_cast<milliseconds>(std::chrono::steady_clock::now() - env->plan_start_time).count() -
                addition_time;
    actions.clear();
    ONLYDEV(g_timer.record_p("_step_s");)
    ...
    ONLYDEV(
        ...
        std::cerr
        << "max_step_time: " << max_step_time << endl;
...
list<pair<int, int>> CUDAPlanner::single_agent_plan(int start, int start_direct, int end)
{
    list<pair<int, int>> path;
...
int CUDAPlanner::getManhattanDistance(int loc1, int loc2)
{
...
bool CUDAPlanner::validateMove(int loc, int loc2)
{
...
list<pair<int, int>> CUDAPlanner::getNeighbors(int location, int direction)
{
...
void CUDAPlanner::initialize(int preprocess_time_limit)
{
    ...
    {
        if (read_param_json<bool>(config["LaCAM2"], "use_orient_in_heuristic") || read_param_json<bool>(config["LaCAM2"], "consider_rotation") || read_param_json<bool>(config["LaCAM2"], "use_slow_executor"))
        {
    ...
    {
        if (read_param_json<bool>(config["LNS"]["LaCAM2"], "use_orient_in_heuristic") || read_param_json<bool>(config["LNS"]["LaCAM2"], "consider_rotation") || read_param_json<bool>(config["LNS"]["LaCAM2"], "use_slow_executor"))
        {
            std::cerr << "no allowed to use orient when compiled with NO_ROT" << std::endl;
            exit(-1);
        }
