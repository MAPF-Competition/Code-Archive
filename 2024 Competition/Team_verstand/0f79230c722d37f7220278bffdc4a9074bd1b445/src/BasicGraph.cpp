...
/**
* @brief: 从指定的起始位置（root_location）计算每个地图点到该位置的最短路径估计（heuristics）
 * @param int root_location: 指定的起始位置。
 * @return vector<double> BasicGraph: 该起始位置到所有地图点的距离。
 */
std::vector<double> BasicGraph::compute_heuristics(int root_location) const
{
    // 初始化结果数组 res，长度为图的大小，初始值设为正无穷（DBL_MAX）。
    ...
    {
        for (const auto& neighbor : get_reverse_neighbors(root_state))
        {
            auto* root = new StateTimeAStarNode(State(root_location, -1,
    ...
    {
        auto* root = new StateTimeAStarNode(root_state, 0, 0, nullptr, 0);
        root->open_handle = heap.push(root);  // add root to heap
        nodes.insert(root);       // add root to hash_table (nodes)
    }
    ...
    while (!heap.empty())
    {
        ...
        heap.pop();
        for (const auto& next_state : get_reverse_neighbors(curr->state))
        {
            double next_g_val = curr->g_val + get_weight(next_state.location, curr->state.location);
            auto* next = new StateTimeAStarNode(next_state, next_g_val, 0, nullptr, 0);
