...
namespace RandomScheduler
{
    ...
    void getNewInputs(vector<int> &free_agents, vector<int> &free_tasks, SharedEnvironment *env)
    {
        ...
        for (int i = 0; i < sz; ++i)
        {
            if (env->curr_task_schedule[i] == -1)
            {
                ...
                free_agents.push_back(i);
                free_tasks.push_back(env->curr_task_schedule[i]);
            }
        }
    ...
    void schedule_plan(int time_limit, std::vector<int> &proposed_schedule, SharedEnvironment *env)
    {
        ...
        for (auto location_and_vector_of_id_cost_task : location_to_task)
        {
            ...
            {
                free_tasks_locations_and_costs.push_back(make_pair(location_and_vector_of_id_cost_task.first, (location_to_task[location_and_vector_of_id_cost_task.first][i].first - minimum_cost)*2)); // {location, i-th minimum cost}
