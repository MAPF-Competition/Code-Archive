...
class pyMAPFPlanner:
    ...
    def sample_priority_planner(self, time_limit: int):
        ...
        # Sort agents based on their distance to the goal to prioritize closer ones
        agent_priority = []
        for i in range(self.env.num_of_agents):
            if self.env.goal_locations[i]:
                start = self.env.curr_states[i].location
                goal = self.env.goal_locations[i][0][0]
                distance = self.getManhattanDistance(start, goal)
                agent_priority.append((distance, i))
            else:
                # No goal, prioritize lower
                agent_priority.append((float('inf'), i))

        agent_priority.sort()  # Sort agents by proximity to goal (ascending)

        for _, i in agent_priority:
            path = []

            if not self.env.goal_locations[i]:
                # Agent has no goal, remain idle
                path.append((self.env.curr_states[i].location, self.env.curr_states[i].orientation))
                reservation.add((self.env.curr_states[i].location, -1, 1))
                continue

            # Plan the path using space-time planning
            path = self.space_time_plan(
                self.env.curr_states[i].location,
                self.env.curr_states[i].orientation,
                self.env.goal_locations[i][0][0],
            ...
            if path:
                # Determine the first action based on the planned path
                if path[0][0] != self.env.curr_states[i].location:
                    actions[i] = MAPF.Action.FW
                elif path[0][1] != self.env.curr_states[i].orientation:
                    incr = path[0][1] - self.env.curr_states[i].orientation
                ...
                # Update reservations based on the planned path
                last_loc = -1
                t = 1
                for p in path:
                    reservation.add((p[0], -1, t))
                ...
                # Retry planning with reduced constraints if the first attempt fails
                retry_path = self.space_time_plan(
                    self.env.curr_states[i].location,
                    self.env.curr_states[i].orientation,
                    self.env.goal_locations[i][0][0],
                    set()  # Ignore reservations for retry
                )
                if retry_path:
                    actions[i] = MAPF.Action.FW  # Assume at least forward movement for simplicity
                    # Add reservations for retry path
                    last_loc = -1
                    t = 1
                    for p in retry_path:
                        reservation.add((p[0], -1, t))
