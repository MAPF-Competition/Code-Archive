...
void MAPFPlanner::initialize(int preprocess_time_limit) {
    if (env->map_name == "Paris_1_256.map") {
        if (env->num_of_agents < 2000) {                // Instance #1: city_1500
            ...
            param.values["path_bound"] = 120;    // 100
            ...
            param.values["path_bound"] = 50;     // best 40, prev -1, new -1
        ...
        if (env->num_of_agents < 150) {                 // Instance #4 (random_100)
            ...
            param.values["must_have_moved"] = 6; // 6
            ...
            param.values["fwbonus"] = 0.1;               // .1
            param.values["achievebonus"] = 0.3;          // .3
            param.values["elite"] = .5;                  // .5
            param.values["greedy"] = 0;                  // 0
            // param.halt = true;
        } else {                                        // Instance #8 (random_800)
            param.algorithm = Algorithm::SAT; 
            param.values["barriers"] = 0;
            param.values["enqueue_bound"] = 100;
            param.values["agents_bound"] = 0.9;
            // param.halt = true;
        }
