...
void PlannerLong::init(SharedEnvironment* _env, const Parameters & parameters) {
    ...
    if (parameters.values.count("replan") > 0)
        m_replan = parameters.values.at("replan");
    ...
    // Define alpha, beta and mu for each round
    const int max_rounds = 100;
    double delta_alpha = 0;
    if (parameters.values.count("delta_alpha") > 0)
        delta_alpha = parameters.values.at("delta_alpha");
    double delta_beta = 0;
    if (parameters.values.count("delta_beta") > 0)
        delta_beta = parameters.values.at("delta_beta");
    int delta_mu = 0;
    if (parameters.values.count("delta_mu") > 0)
        delta_mu = parameters.values.at("delta_mu");
    m_alpha_v.resize(max_rounds);
    m_beta_v.resize(max_rounds);
    m_enqueue_bound_v.resize(max_rounds);
    for (int round = 0; round < max_rounds; round++) {
        m_alpha_v[round] = m_alpha + delta_alpha * round;
        m_beta_v[round] = m_beta + delta_beta * round;
        m_enqueue_bound_v[round] = m_enqueue_bound + delta_mu * round;
    }


    cout << "PlannerLong("
        << "alpha: " << m_alpha << "[d:" << delta_alpha << "]"
        << ", beta: " << m_beta << "[d:" << delta_beta << "]"
        << ", enqueue_bound: " << m_enqueue_bound  << "[d:" << delta_mu << "]"
        << ", replan: " << m_replan
...
void PlannerLong::plan(int time_limit, vector<Action> & actions) {
    ...
    m_round = 0;
    while (!backup_queue.empty() && m_round < m_alpha_v.size()) {
        // Reset the data
        queue = backup_queue;
    ...
    cout << "-- Time: " << time_diff(m_start_time) << " ; rounds: " << m_round << endl;
    print_elapsed_time();
    m_timestep++;
...
void PlannerLong::fill_queue(std::queue<int> & queue) {
    ...
    // Complete the queue to have more agents
    std::random_device rd;
    std::mt19937 generator(rd());
    std::shuffle(other_agents.begin(), other_agents.end(), generator);
    const int agents_to_add = std::max(0, m_replan - static_cast<int>(queue.size()));
    for (int i = 0; i < agents_to_add; i++) {
        int ca = other_agents.at(i);
...
bool PlannerLong::try_planning(std::queue<int> & queue) {
    ...
    while (!queue.empty()) {
        if (m_round > 0 && time_diff(m_start_time) > m_time_limit - 0.05) {
            cout << "-- " << count << " agents planned in " << time_diff(m_start_time) << " seconds (round " << m_round << ", interrupted)" << endl;
            return false;
        }
    ...
    cout << "-- " << count << " agents planned in " << time_diff(m_start_time) << " seconds (round " << m_round << ")" << endl;
    return true;
}
...
std::vector<int> PlannerLong::plan_path(int agent, const Position &start) {
    ...
    while (!open_list.empty()) {
        ...
        for (const auto &[neighbor, conflicts] : neighborhood) {
            ...
            const double next_g = (curr->time - m_timestep) + m_alpha_v.at(m_round) * confs.first;
...
std::pair<int, std::bitset<200>> PlannerLong::sum_conflicts(Node * curr, const std::vector<int> & conflicts) const {
    ...
    for (int a : conflicts) {
        if (!next_conflicts.test(a)) {
            ...
            if (!m_dummy.at(a))
                cost += std::pow(m_enqueued.at(a) + 1, m_beta_v.at(m_round));
        }
    }
...
void PlannerLong::forbid_position(int agent, const std::vector<int> & conflicts) {
    if (m_enqueued.at(agent) < m_enqueue_bound_v.at(m_round))
        return;
    if (conflicts.empty())
        return;
