...
// 	else 
// 		min_heuristic = get_h(lns.env, target, lns.tasks.at(ai));
...
bool causalPIBT(int curr_id, int higher_id,std::vector<State>& prev_states,
	 std::vector<State>& next_states,
	  ...
	  // TrajLNS& lns
	  std::shared_ptr<Dist2PathHeuristicTable> & heuristic_table
	  ){
	...
	int next[4] = { prev_loc + 1,prev_loc + heuristic_table->env->cols, prev_loc - 1, prev_loc - heuristic_table->env->cols};
	int orien_next_v = next[prev_orientation];

	assert(prev_loc >= 0 && prev_loc < heuristic_table->env->map.size());
	...
	getNeighborLocs(&global_neighbors,neighbors,prev_loc);
	for (auto& neighbor: neighbors){

		assert(validateMove(prev_loc, neighbor, heuristic_table->env));

		// int min_heuristic = get_gp_h(heuristic_table, curr_id, neighbor);
		int min_heuristic = heuristic_table->get(curr_id, neighbor, -1);

		successors.emplace_back(neighbor,min_heuristic,-1,rand());
	}

	// int wait_heuristic = get_gp_h(heuristic_table, curr_id, prev_loc);
	int wait_heuristic = heuristic_table->get(curr_id, prev_loc, -1);
	...
	std::sort(successors.begin(), successors.end(), 
		[&](PIBT_C& a, PIBT_C& b)
		{
			int diff[4] = {1,heuristic_table->env->cols,-1,-heuristic_table->env->cols};
    ...
    for (auto& next: successors){
		if(occupied[next.location])
			continue;
		assert(validateMove(prev_loc, next.location, heuristic_table->env));
        ...
        if (prev_decision.at(next.location) != -1 && 
            ...
            if (!causalPIBT(lower_id,curr_id,prev_states,next_states, prev_decision,decision, occupied, heuristic_table)){
				continue;
            }
        }
