    ...
    MemoryPool& mem, int start, int goal, Neighbors* ns)
{
    ...
    h = UTIL::static_heuristic_table_no_rot->get(start, goal);
    ...
    while (open.size() > 0){
        ...
        for (int i=0; i<4; i++){
            ...
            cost = curr->g+UTIL::static_heuristic_table_no_rot->get_edge_weight(curr->id,next);
            ...
            h = UTIL::static_heuristic_table_no_rot->get(next, goal);
            ...
            if (!mem.has_node(next)){
                ...
                if (!existing->is_closed()){
                    ...
                    if (re(temp_node,*existing)){ 
                        std::cerr << "error in astar: re-expansion" << std::endl;
                        std::cerr << "start node"<< start << std::endl;
                        std::cerr << "goal node"<< goal << std::endl;
                        std::cerr << "temp_node: " << temp_node.id << "," << temp_node.g << "," << temp_node.h << "," << temp_node.op_flow << "," << temp_node.all_vertex_flow<< ","<< temp_node.tie_breaker << std::endl;
                        std::cerr << "existing: " << existing->id << "," << existing->g << "," << existing->h << "," << existing->op_flow << "," << existing->all_vertex_flow<<","<<existing->tie_breaker << std::endl;
                        std::cerr << "temp_node's parent: " << curr->id << "," << curr->g << "," << curr->h << "," << curr->op_flow << "," << curr->all_vertex_flow<< ","<< curr->tie_breaker << std::endl;
                        if (existing->parent != nullptr)
                            std::cerr << "existings's parent: " << existing->parent->id << "," << existing->parent->g << "," << existing->parent->h << "," << existing->parent->op_flow << "," << existing->parent->all_vertex_flow<<","<<existing->parent->tie_breaker << std::endl;
                        else 
                            std::cerr << "existings's parent: nullptr" << std::endl;
