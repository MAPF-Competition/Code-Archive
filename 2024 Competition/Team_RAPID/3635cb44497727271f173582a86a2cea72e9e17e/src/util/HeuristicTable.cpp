...
HeuristicTable::HeuristicTable(SharedEnvironment *_env, const std::shared_ptr<std::vector<float>> &map_weights, bool consider_rotation) : env(*_env),
                                                                                                                                          action_model(_env),
                                                                                                                                          ...
                                                                                                                                          map_weights(map_weights)
{
    ...
    //为每个地图位置建立一个索引数组 loc_idxs，大小与 env.map 一致。该数组用来存储每个位置在“空闲位置数组”中的索引；若当前位置为障碍，则保持 -1。
    ...
    if (consider_rotation)
        ...
        //分成两部分，优化内存
        //否则需要分配一个大小为 state_size*state_size 的数组
};
...
void HeuristicTable::compute_weighted_heuristics()
{
    ...
    //分配一个指针数组 planners，用于存储每个线程对应的 SpatialAStar 规划器。
...
//使用 OpenMP 指令 #pragma omp parallel for 将接下来的 for 循环并行化，调度策略为动态调度，每次分配一个迭代（schedule(dynamic, 1)）。
...
//计算从一个特定起始位置（由 start_loc_idx 指定）出发的加权启发式值。
    ...
    else
    {
        ...
        for (int start_orient = 0; start_orient < n_orientations; ++start_orient)
        {
            ...
            for (int loc_idx = 0; loc_idx < loc_size; ++loc_idx)
            {
                for (int orient = 0; orient < n_orientations; ++orient)
                {
                    ...
                    {
                        main_heuristics[main_idx] = cost;
                    }//对于同一目标位置（无论目标朝向如何），记录所有起始朝向下的最小代价，即取最小值。
                }
            }
        }
        ...
        for (int start_orient = 0; start_orient < n_orientations; ++start_orient)
        {
            for (int loc_idx = 0; loc_idx < loc_size; ++loc_idx)
            {
                ...
                size_t main_idx = start_loc_idx * loc_size + loc_idx;
                float diff = cost - main_heuristics[main_idx];//表示当前起始朝向下与全局最小代价之间的差值。
                if (diff < 0)
                {
...
//用于将某个起始位置 start_loc 对应的主启发式信息写入文件。
...
// }
//此函数用于预处理，即检查是否已存在预先计算并存储的启发式数据文件；如果存在则加载，否则进行计算。
