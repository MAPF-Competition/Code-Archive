...
namespace LaCAM2
{
  ...
  Instance LaCAM2Solver::build_instance(SharedEnvironment &env, std::vector<Path> *precomputed_paths)
  {
    ...
    for (int i = 0; i < env.num_of_agents; ++i)
    {
      ...
      {
        env.goal_locations[i].emplace_back(std::make_pair(env.curr_states[i].location, 0));
      }
  ...
  void LaCAM2Solver::plan(SharedEnvironment &env, std::vector<Path> *precomputed_paths, std::vector<::State> *starts, std::vector<::State> *goals)
  {
