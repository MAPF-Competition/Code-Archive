...
void RandomPlannerSIPP::initialize(int preprocess_time_limit)
{
    shift_id = 0;
    shift_id1 = 0;
...
void RandomPlannerSIPP::dfs_one_cycle(int i, int j, int dir, bool ccw, vector<int> &mp, int id, int cnt, int i_st, int j_st, vector<int> &vis)
{
...
void RandomPlannerSIPP::bfs_outer_cycle()
{
...
void RandomPlannerSIPP::findSafePlaces()
{

    for(int i=0; i<env->rows; ++i){
        for(int j=0; j<env->cols; ++j){
            if(env->map[i*env->cols+j]==0){
                if(is_safe_place(freeCells[i*env->cols+j])){
                    safePlace[i*env->cols+j] = 1;
                    safePlaceFree[freeCells[i*env->cols+j]] = 1;
                }
            }
        }
    }
...
void RandomPlannerSIPP::compressFreeCells(){
    numOfFreeCells = 0;
    ...
    occupied = new bool [numOfFreeCells]();
...
void RandomPlannerSIPP::genAllNeighbors()
{
    int cnt = 0;
...
void RandomPlannerSIPP::writeSafePlaces(){
    std::ofstream out;
    out.open("safe_places2.txt");
...
// void RandomPlannerSIPP::fillMap()
// {
//     map.width = env->cols;
//     map.height = env->rows;
//     int cnt = 0;
...
//     map.generate_moves();
// }
// void RandomPlannerSIPP::fillTask()
// {
//     task.agents.clear();
...
void RandomPlannerSIPP::plan(int time_limit, vector<Action> &actions)
{
    ++timeStep;
    ...
    for(int i=0; i<env->num_of_agents; ++i){
        ...
        occupied[curr_states[i].first] = 1;
    ...
    for(int repeat = 0; repeat < 1; ++repeat){
        for (int ii=0; ii<v.size(); ++ii)
        {
            ...
            int i = v[ii].second;
            bool l = 0;
            // cnt += old_paths[i].size()>0 && is_safe_place(old_paths[i].back().first.first);
            for(auto it:old_paths[i]){
                if(it.first.first == goal_locations[i]){
                    l = 1;
                    break;
    ...
    int cnt = 0, sm_paths = 0;
    vector<int> times_paths;
    for (int i = 0; i < env->num_of_agents; ++i)
    {
        bool l = 0;
        // cnt += old_paths[i].size()>0 && is_safe_place(old_paths[i].back().first.first);
        for(auto it:old_paths[i]){
            if(it.first.first == goal_locations[i]){
                ++cnt;
                l = 1;
                times_paths.push_back(it.second.first);
                break;
            }
        }
        if(l){
    ...
    checkReservations();
    sort(times_paths.begin(), times_paths.end());
    for(auto it:times_paths){
        cout<<it-timeStep<<" ";
    }
    cout<<endl;
    cout<<cnt<<"/"<<sm_paths<<"//"<<v.size()<<"//"<<env->num_of_agents<<endl;
    return;
}

void RandomPlannerSIPP::single_agent_plan(int start, int start_direct, int end, list<pair<pair<int, int>,pair<int, int>>> &path)
{
    // if(mapFree[end]){
    //     return;
    // }
    ...
    open_list.push(s);
    AstarNode ans_node, second_ans, third_ans;
    bool solved = false, solved_second = false, goalWasOnceVisited = false, solved_third = false;
    int par;
    ...
    while (!open_list.empty())
    {
        ...
        if (mapFree[curr.location] == INT_MAX && (map[curr.location].empty() || map[curr.location].upper_bound({curr.g,curr.g}) == map[curr.location].end()))
        {
            ...
            //     }
            // }
            if(safePlaceFree[curr.location]){
                solved_second = true;
                second_ans = curr;
            }
        }
        ...
        if (!closed_list[id*2+curr.goalWasVisited].empty())
        {
            ...
            if(it_node_prev != closed_list[id*2+curr.goalWasVisited].end())continue;
            if (it_node_prev != closed_list[id*2+curr.goalWasVisited].begin())
            {
                ...
                if(*it_node_prev == curr.g)continue;
                ...
                { // no block times at this location.
                    ...
                    if (*it_node_prev > it_block_prev->second)// && it_block_prev!=--map[curr.location].end())
                        continue;
                }
            }
        }
    ...
    // cout<<solved<<" "<<solved_second<<" "<<goalWasOnceVisited<<endl;
    // if (std::chrono::duration_cast<microseconds>(high_resolution_clock::now() - start_plan_time).count() <= max_time_limit){
        if(solved){
            cout<<"Solved"<<endl;
        ...
        else if(solved_third){
            // cout<<"Third solved"<<endl;
            path.emplace_front(make_pair(make_pair(third_ans.location, third_ans.direction), make_pair(third_ans.g, third_ans.g)));
            while (third_ans.parent != -1)
            {
                auto pa = all_nodes[third_ans.parent];
                path.emplace_front(make_pair(make_pair(pa.location, pa.direction), make_pair(pa.g, third_ans.g-1)));
                third_ans = pa;
            }
        }
    // }
...
void RandomPlannerSIPP::reserve(const std::list<pair<pair<int, int>,pair<int, int>>> &path)
{
...
void RandomPlannerSIPP::del_reservations(const std::list<pair<pair<int, int>, pair<int, int>>> &path)
{
    if(path.size()==0){
        return;
    }
...
void RandomPlannerSIPP::del_first_reservation(std::list<pair<pair<int, int>, pair<int, int>>> &path)
{
    auto loc = path.front();
...
int RandomPlannerSIPP::getManhattanDistance(int loc1, int loc2)
{
...
bool RandomPlannerSIPP::check(list<pair<int, int>> &path)
{
    int t = 1;
    for (auto it : path)
    {
...
void RandomPlannerSIPP::dfs_check_visit(int x, int is_start, vector<bool>&vis, int&cnt){
    // cout<<x<<endl;
    if(env->map[x]){
        return;
    }
    if(vis[x])return;
    vis[x]=1;
    // if(!is_start && mapFree[freeCells[x]]<INT_MAX){
    if(!is_start && safePlace[x]){
        ++cnt;
        return;
    }
    // if(!safePlace[x])
    ...
    for (int k = 0; k < 4; ++k)
    {
        ...
        if (ii < env->rows && ii >= 0 && jj < env->cols && jj >= 0)
        {
            dfs_check_visit(ii*env->cols+jj, 0, vis, cnt);
        }
    }
}

bool RandomPlannerSIPP::is_safe_place(int loc)
{
    int di[4] = {0, 1, 0, -1}, dj[4] = {1, 0, -1, 0};
    int i = row[loc];
    int j = col[loc];
    bool l = 1;
    env->map[i*env->cols+j]=1;
    std::set<int> st;
    for(int k=0; k<4; ++k){
        int ii = i + di[k];
        int jj = j + dj[k];
        if(ii>=0 && ii<env->rows && jj>=0 && jj<env->cols && (env->map[ii*env->cols+jj]==0)){
            int id = ii * env->cols + jj;
            int tmp1 = 0;
            vector<bool> vis = vector<bool>(env->cols*env->rows, 0);
            dfs_check_visit(id, 1, vis, tmp1);
            // cout<<ii<<" "<<jj<<" "<<tmp1<<" "<<tmp<<endl;
            st.insert(tmp1);
        }
    }
    env->map[i*env->cols+j]=0;
    return st.size()==1 && *st.begin()==numOfFreeCells-1;


    // return safePlaceFree[loc];
    ...
    for(int k=0; k<8; ++k){
        ...
        int jj = y+dy[k];
        if(ii>=0 && ii<env->rows && jj>=0 && jj<env->cols && (env->map[ii*env->cols+jj]==0) && 
        !((dx[k]<0 || (dy[k]<0 && dx[k]==0)) && safePlaceFree[freeCells[ii*env->cols+jj]])){//} && mapFree[freeCells[ii*env->cols+jj]]==INT_MAX)){
            if(k%2==0)
                l1 = 0;
        }
        else{
...
list<pair<int, pair<int, int>>> RandomPlannerSIPP::getNeighbors(int location, int direction, int t)
{
...
void RandomPlannerSIPP::checkReservations()
{
    int tmp = 0;
    for(int i=0; i<env->num_of_agents; ++i){
