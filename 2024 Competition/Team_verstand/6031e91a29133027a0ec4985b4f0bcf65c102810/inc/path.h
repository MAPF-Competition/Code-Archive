...
public:
    ...
    // constructor
    TrajLNS(SharedEnvironment* env, std::vector<HeuristicTable>& heuristics,
            Neighbors& neighbors):
            env(env),
    ...
    // 根据输入的num highway初始化。num highway替换原来的num agents
    TrajLNS(SharedEnvironment* env, std::vector<HeuristicTable>& heuristics,
            Neighbors& neighbors, int _num_highway):
            env(env),
            trajs(_num_highway),
            tasks(_num_highway),
            flow(env->map.size(),Int4({0,0,0,0})), heuristics(heuristics),
            traj_dists(_num_highway),goal_nodes(_num_highway),
            fw_metrics(_num_highway),neighbors(neighbors){
    };
