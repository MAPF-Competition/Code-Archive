...
void Planner::init(SharedEnvironment* _env, const Parameters & _parameters) {
    ...
    // Only needed if we want to recompute the distances from the barriers
    // write_directions_map();
    // make_distance_map_json(); // @note This function uses too much memory for large maps
}
...
/// @brief Build the distance map (to check that it is ok)
/// @param location The location from which we compute the distance
void Planner::make_distance_map(int location) {
    m_distance_map = compute_distance(location);
    // write_distance_map_json(); // @debug Check with distance-viewer that this works
}
...
void Planner::read_distance_sites() {
    ...
    // Read the sites
    std::string json_fn = env->file_storage_path + "/sites.json";
    ...
    for (int i = 0; i < sites.size(); i++) {
        ...
        make_distance_map(sites.at(i));
        for (int j = 0; j < sites.size(); j++) {
            m_sites_distance.at(j).at(i) = m_distance_map.at(sites.at(j));
        }
    }
    m_distance_map.clear();
...
void Planner::make_labels(const std::vector<int> & sites) {
    ...
    while (!queue.empty()) {
        ...
        const std::list<int> neighbors = location_neighbors(cur_loc);
...
void Planner::write_directions_map() {
    ...
    for (int i = 0; i < n_pixels; i++) {
        std::vector<char> cur_dir = compute_directions(m_location_at_index.at(i));
        ...
        if (i % 100 == 0)
            cout << "Directions from location with index " << i << "/" << n_pixels << " done\r" << endl;
    }
    file.close();
...
std::vector<char> Planner::compute_directions(int location) const {
    ...
    // cout << dist[934] << "  " << dist[935] << "  " << dist[1464] << endl;
...
/**
 * @brief Planner::compute_distance
 * Compute the distance map to a given location
 */
 ...
 */
std::list<int> Planner::location_neighbors(int loc) const {
...
/// @brief Write the current distance_map vector to a JSON file
/// so that we can open it with distance-viewer
void Planner::write_distance_map_json() const {
    std::vector<std::vector<int>> distances(1);
    distances.at(0) = m_distance_map;
    // write JSON file
    nlohmann::json data;
    data["distances"] = distances;
