...
comment:  卧槽，原来已经是TFO算法了。我还以为是PIBT算法。这个优化难度可就大了。
...
- **Don't change the definitions** for the `TaskScheduler::initialize()` and `TaskScheduler::plan()` functions. Except for this, you are free to add new members/functions to the `TaskScheduler` class. (comment:  推测这里的意思是不可以改函数的名称和接口，但是可以改内容。吾人修改内容后上传成功了)
...
- Implement your preprocessing in the function `MAPFPlanner::initialize()` that is provided to you. (comment:  原来可以修改初始化的代码)
