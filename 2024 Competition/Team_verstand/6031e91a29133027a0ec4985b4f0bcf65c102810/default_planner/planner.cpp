...
namespace DefaultPlanner{
    ...
    void initialize(int preprocess_time_limit, SharedEnvironment* env){
            ...
            // assign initial priority to each agent
    ...
    void plan(int time_limit,vector<Action> & actions, SharedEnvironment* env){
        ...
        // 2. 初始化智能体初始状态和目标位置
        // 如果是初始时间步，则为每个智能体记录初始位置，作为默认目标位置（dummy_goals）。
        ...
        for(int i=0; i<env->num_of_agents; i++)
        {
            ...
            // 如果某个智能体没有明确目标，使用默认目标位置（dummy_goals）。
            ...
            if (prev_states[i].location == decided[i].loc){
                decided[i].state = DoneState::DONE;
            }
