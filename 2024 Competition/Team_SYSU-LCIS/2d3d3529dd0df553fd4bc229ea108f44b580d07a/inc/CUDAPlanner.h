...
public:
  ...
  CUDAPlanner(SharedEnvironment *env, CUDAHeuristics *cuda_heuristics) : env(env), cuda_heuristics(cuda_heuristics) {};
  CUDAPlanner()
  {
    env = new SharedEnvironment();
    cuda_heuristics = new CUDAHeuristics(cuda_id, env);
  };
  virtual ~CUDAPlanner() { delete env; };
...
public:
  ...
  CUDAPlanner(SharedEnvironment *env) : env(env) {};
  CUDAPlanner() { env = new SharedEnvironment(); };
  virtual ~CUDAPlanner() { delete env; };
