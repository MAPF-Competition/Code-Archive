...
public:
	...
	/////////////////////////////////////////////////////////////////////////////////////////
	// set params
	void setHeuristicType(EECBS_PREFIX_heuristics_type h, EECBS_PREFIX_heuristics_type h_hat)
	{
	...
	CBS(const EECBS_PREFIX_Instance& instance, bool sipp, int screen);
	...
	int num_of_agents;
	vector<EECBS_PREFIX_Path*> paths;
protected:
	...
	vector<EECBS_PREFIX_Path> paths_found_initially;  // contain initial paths found
	...
	set<int> getInvalidAgents(const list<EECBS_PREFIX_Constraint>& constraints); // return agents that violate the constraints
	//conflicts
	void findConflicts(HLNode& curr);
	void findConflicts(HLNode& curr, int a1, int a2);
	shared_ptr<EECBS_PREFIX_Conflict> chooseConflict(const HLNode &node) const;
	static void copyConflicts(const list<shared_ptr<EECBS_PREFIX_Conflict>>& conflicts,
		list<shared_ptr<EECBS_PREFIX_Conflict>>& copy, const list<int>& excluded_agent) ;
	void removeLowPriorityConflicts(list<shared_ptr<EECBS_PREFIX_Conflict>>& conflicts) const;
	void computeSecondPriorityForConflict(EECBS_PREFIX_Conflict& conflict, const HLNode& node);
	...
	void computeConflictPriority(shared_ptr<EECBS_PREFIX_Conflict>& con, CBSNode& node); // check the conflict is cardinal, semi-cardinal or non-cardinal
