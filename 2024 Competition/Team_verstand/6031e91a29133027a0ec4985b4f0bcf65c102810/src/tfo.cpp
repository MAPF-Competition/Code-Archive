//
// Created by take_ on 25-1-9.
//
#include "tfo.h"
#include <random>
#include <unordered_set>


std::mt19937 g(0);

/**
* @brief: 从给定的轨迹中移除agent对 flow 的影响: 减少路径上的每一步流量，从而更新流量表。
 * @param TrajLNS& lns: 保存移除agent后的SOC和flow。
 * @param int agent: 需要移除其轨迹影响的具体智能体编号。
 * @return void
 * comment: remove flow for each location's outgoing edge according to the traj
 */
void remove_traj(TrajLNS& lns, int agent){
    // 移除agent后的SOC
    lns.soc -= lns.trajs[agent].size() - 1;
    // 如果路径只有一个点则不影响
    if (lns.trajs[agent].size() <= 1){
        return;
    }
    int loc, prev_loc, diff, d, to;

    // 获取路径的长度，作为循环的终止条件。
    to = lns.trajs[agent].size();
    // 遍历轨迹中所有的路径点
    for (int j = 1; j < to; j++){
        loc = lns.trajs[agent][j]; // 当前路径点
        prev_loc = lns.trajs[agent][j-1]; // 前一个路径点
        diff = loc - prev_loc;
        d = get_d(diff, lns.env); // 根据偏移获取方向

        lns.flow[prev_loc].d[d] -= 1; // 路径点方向的流量-1
    }
}

/**
* @brief: 将某个智能体（agent）的轨迹（traj）信息添加到 TrajLNS 对象中，从而更新与路径相关的代价和流量数据。
 * @param TrajLNS& lns: 保存加入agent后的SOC和flow。
 * @param int agent: 需要加入其轨迹影响的具体智能体编号。
 * @return void
 */
void add_traj(TrajLNS& lns, int agent){
    // update last replan time for agent
    lns.fw_metrics[agent].last_replan_t = lns.env->curr_timestep;

    // 增加智能体轨迹的代价到总代价（soc）。
    lns.soc += lns.trajs[agent].size() - 1;
    if (lns.trajs[agent].size() <= 1){
        return;
    }
    int loc, prev_loc, diff, d;
    for (int j = 1; j < lns.trajs[agent].size(); j++){
        loc = lns.trajs[agent][j];
        prev_loc = lns.trajs[agent][j-1];
        diff = loc - prev_loc;
        d = get_d(diff, lns.env);

        lns.flow[prev_loc].d[d] += 1; // 更新轨迹中每一步的出边流量。

    }
}

/**
* @brief: 检测哪些智能体的当前位置已经偏离了其轨迹（轨迹偏差，deviation），并根据偏离程度对这些智能体进行排序。
 * @param TrajLNS& lns: 保存加入agent后的SOC和flow。
 * @return void
 * comment: 感觉可以用在highway
 */
void get_deviation(TrajLNS& lns){
    lns.deviation_agents.clear();
    for  (int i=0; i< lns.env->num_of_agents;i++){
        if (lns.traj_dists[i].empty() || lns.trajs[i].empty())
            continue;

        std::pair<int,int> dists =  get_source_2_path(lns.traj_dists[i], lns.env, lns.env->curr_states[i].location, &(lns.neighbors));

        if (dists.first > 0){
            lns.deviation_agents.emplace_back(dists.first, i);
        }
    }

    std::sort(lns.deviation_agents.begin(), lns.deviation_agents.end(),
              [](std::pair<int,int>& a, std::pair<int,int>& b)
              {
                  return a.first > b.first;
              });
}

/**
* @brief: 更新agent从当前位置到预设轨迹的距离
 * @param TrajLNS& lns: 保存更新后的fw_metrics。
 * @return void
 */
void update_fw_metrics(TrajLNS& lns){
    // 1. 遍历每个智能体
    for  (int i=0; i< lns.env->num_of_agents;i++){
        lns.fw_metrics[i].id = i;
        lns.fw_metrics[i].rand = rand();
        lns.fw_metrics[i].deviation = 0;
        if (lns.traj_dists[i].empty() || lns.trajs[i].empty())
            continue;
        std::pair<int,int> dists =  get_source_2_path(lns.traj_dists[i], lns.env, lns.env->curr_states[i].location, &(lns.neighbors));
        assert(dists.first >= 0 );
        lns.fw_metrics[i].deviation = dists.first; // agent i从当前位置到预设轨迹的距离
    }
    return;
}

/**
* @brief: 利用一种 Frank-Wolfe 样式的迭代方法，为一组智能体（agents）重新规划轨迹，同时考虑路径上的偏离程度、上次重规划时间等因素。
 * 在给定的时间限制内（timelimit），它会逐步更新需要重规划的智能体的路径。
 * @param TrajLNS& lns: 存储智能体的路径（trajs）、流量信息（flow）、启发式（heuristics）、当前的轨迹偏差（fw_metrics）等。
 * @param std::unordered_set<int>& updated: 保存被更新轨迹的智能体 ID 集合。
 * @param TimePoint timelimit: 时间限制，函数运行的时间不能超过这个限制。
 * @return void
 */
void frank_wolfe(TrajLNS& lns, const vector<vector<int>>& _extra_weights,std::unordered_set<int>& updated, TimePoint timelimit){
    // 计算每个智能体当前位置与其预设轨迹的偏差（deviation）. 到轨迹就可以了, 不需要到终点
    update_fw_metrics(lns);
    std::vector<FW_Metric> replan_order = lns.fw_metrics;
    assert(replan_order.size() == lns.env->num_of_agents);

    // 智能体重规划按照偏离程度和上次规划时间排序
    std::sort(replan_order.begin(), replan_order.end(),
              [](FW_Metric& a, FW_Metric& b)
              {
                  // 偏离轨迹的距离（deviation）越大，优先级越高。
                  // comment: 离得远, 说明空间效性降低了, 重规划需求更大
                  if (a.deviation > b.deviation)
                      return true;
                  else if ( a.deviation < b.deviation)
                      return false;
                  // 上次重规划时间（last_replan_t）越早，优先级越高。
                  // comment: 时间隔得远, 说明时效性降低了, 重规划需求更大
                  if (a.last_replan_t < b.last_replan_t)
                      return true;
                  else if (a.last_replan_t > b.last_replan_t)
                      return false;

                  return a.rand > b.rand;
              });

    int count=0;
    int agent, index;
    // 在时间限制内迭代重规划
    while (std::chrono::steady_clock::now() < timelimit){
        index = count % lns.env->num_of_agents; // 循环完一次, 又会掉头规划最高优先级
        agent = replan_order[index].id;
        count++;
        if (lns.traj_dists[agent].empty() || lns.trajs[agent].empty()){
            continue;
        }
        remove_traj(lns, agent); // 移除旧轨迹对流量的影响
        update_traj(lns, _extra_weights, agent); // 更新轨迹

    }

    // cout << "count: " << count << endl;
}

/**
* @brief: 更新第 i 个智能体的路径到末尾距离数据。
 * @param TrajLNS& lns: 保存更新后的第 i 个智能体的路径到末尾距离数据。
 * @return void
 */
void update_dist_2_path(TrajLNS& lns, int i){
    init_dist_2_path(lns.traj_dists[i], lns.env, lns.trajs[i]);
}

//compute distance table for each traj
void init_dist_table(TrajLNS& lns, int amount){

    int count = 0;
    for (int i=0 ; i <lns.env->num_of_agents; i++){
        // std::cout<<i<<";";
        if (count >= amount){
            break;
        }
        if(!lns.trajs[i].empty() && lns.trajs[i].size() == get_heuristic(lns.heuristics[lns.trajs[i].back()], lns.env,lns.trajs[i].front(),&(lns.neighbors)))
            continue;
        if(!lns.trajs[i].empty() && lns.traj_dists[i].empty()){
            init_dist_2_path(lns.traj_dists[i], lns.env, lns.trajs[i]);
            count++;
            lns.dist2path_inited++;
        }

    }
}

/**
* @brief: update traj and distance table for agent i
 * @param TrajLNS& lns: 保存更新后的路径等信息。
 * @param int i: 指定的agent index。
 * @return void
 */
void update_traj(TrajLNS& lns, const vector<vector<int>>& _extra_weights, int i){
    int start = lns.env->curr_states[i].location;
    int goal = lns.tasks[i];

    /*
    start = 65 * lns.env->cols + 1;
    goal = 62 * lns.env->cols + 255;
    cout << "goal: " << lns.env->map[goal] << endl;
     */

    lns.goal_nodes[i] = astar(lns.env,lns.flow, lns.heuristics[goal],lns.trajs[i],
                              _extra_weights, lns.mem,start,goal, &(lns.neighbors));
    add_traj(lns,i);
    update_dist_2_path(lns,i);
}

