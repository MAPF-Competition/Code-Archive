...
EECBS_PREFIX_Path SIPP::findPath(const ConstraintTable& constraint_table)
{
    reset();
    //EECBS_PREFIX_Path path = findNoCollisionPath(constraint_table);
    //if (!path.empty())
    //    return path;
    ReservationTable reservation_table(constraint_table, goal_location);
    EECBS_PREFIX_Path path;
...
EECBS_PREFIX_Path SIPP::findOptimalPath(const HLNode& node, const ConstraintTable& initial_constraints,
...
pair<EECBS_PREFIX_Path, int> SIPP::findSuboptimalPath(const HLNode& node, const ConstraintTable& initial_constraints,
                                         const vector<EECBS_PREFIX_Path*>& paths, int agent, int lowerbound, double w)
{
    reset();
    this->w = w;
...
/*EECBS_PREFIX_Path SIPP::findNoCollisionPath(const ConstraintTable& constraint_table)
{
    reset();
    ReservationTable reservation_table(constraint_table, goal_location);
    EECBS_PREFIX_Path path;
