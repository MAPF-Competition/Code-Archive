               ...
               ReservationTable& rt)
{
	...
	if (h_val > INT_MAX)
	{
	    cout << "The start ("<<G.get_row(start.location)<<","<<G.get_col(start.location)<<") and goal locations ("<<G.get_row(goal_location[0].first)<<","<<G.get_col(goal_location[0].first)<<") are disconnected!" << endl;
		return Path();
	}
