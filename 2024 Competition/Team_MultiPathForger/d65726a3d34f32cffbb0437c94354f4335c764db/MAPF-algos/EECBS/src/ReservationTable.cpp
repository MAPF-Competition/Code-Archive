...
void ReservationTable::updateSIT(int location)
{
    ...
    if (location == goal_location) // we need to divide the same intervals into 2 parts [0, length_min) and [length_min, length_max + 1)
    {
        ...
        sit[location].emplace_back(constraint_table.length_min, min(constraint_table.length_max + 1, EECBS_MAX_TIMESTEP), false);
    }
    else
    {
        sit[location].emplace_back(0, min(constraint_table.length_max, EECBS_MAX_TIMESTEP - 1) + 1, false);
    }
    ...
    if (!constraint_table.cat.empty())
    {
        ...
        if (constraint_table.cat_goals[location] < EECBS_MAX_TIMESTEP)
            insertSoftConstraint2SIT(location, constraint_table.cat_goals[location], EECBS_MAX_TIMESTEP + 1);
    }
}
...
{
    if (t_min >= min(constraint_table.length_max, EECBS_MAX_TIMESTEP - 1) + 1)
        return false;
    if (sit[location].empty())
        updateSIT(location);
