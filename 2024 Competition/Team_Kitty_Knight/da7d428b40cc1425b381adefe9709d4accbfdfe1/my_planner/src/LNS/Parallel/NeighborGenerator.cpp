...
NeighborGenerator::NeighborGenerator(
    Instance & instance, std::shared_ptr<MyPlanner::Dist2PathHeuristicTable> HT, PathTable & path_table, 
    std::vector<Agent> & agents, std::shared_ptr<std::vector<AgentInfo> > agent_infos,
...
void NeighborGenerator::generate_parallel(const UTIL::TimeLimiter & time_limiter) {
...
Neighbor NeighborGenerator::generate(const UTIL::TimeLimiter & time_limiter,int idx) {
    ...
    while (!succ){
        ...
        switch (destroy_strategy)
        {
            ...
            default:
                std::cerr << "Wrong neighbor generation strategy" << endl;
                exit(-1);
        }
        ...
        if (!succ) {      
            ...
            if (screen>=1)
                std::cerr<<"generate neighbors failed"<<endl;
        }
    }
...
void NeighborGenerator::chooseDestroyHeuristicbyALNS() {
    ...
    switch (selected_neighbor)
    {
        ...
        default : std::cerr << "ERROR" << endl; exit(-1);
    }
}
...
#ifndef NO_ROT
             << "(" << HT->get(agents[a].id,agents[a].getStartLocation(),agents[a].getStartOrientation())
#else
             << "(" << HT->get(agents[a].id,agents[a].getStartLocation())
#endif
...
void NeighborGenerator::randomWalk(int agent_id, int start_timestep, set<int>& conflicting_agents, int neighbor_size)
{
...
#ifndef NO_ROT
                float next_h_val = HT->get(agent_id, next_loc, next_orient);
#else
                float next_h_val = HT->get(agent_id, next_loc);
#endif
    ...
    //             // float action_cost = agent.get_action_cost(loc, orient, next_loc, next_orient, HT);
    //             float next_h_val = HT->get(agent_idx, next_loc, next_orient,instance.goal_locations[agent_id]);
    //             // if we can find a path with a smaller distance, we try to see who becomes the obstacle.
    //             // TODO(rivers): this is not correct if we have weighted distance map
