  ...
  for (auto it = sit[location].begin(); it != sit[location].end(); ++it) {
    ...
    if (i_min < t_min && i_max <= t_max) {
      if (it != sit[location].end() and std::next(it) != sit[location].end() and
          ...
          i_max == get<0>(*std::next(it)) and get<2>(*std::next(it)))
      ...
      if (it != sit[location].begin() and
          (location != goal_location || i_min != constraint_table.length_min) and
          i_min == get<1>(*std::prev(it)) and get<2>(*std::prev(it)))
...
void ReservationTable::updateSIT(int location) {
  ...
  // we need to divide the same intervals into 2 parts [0, length_min) and [length_min, length_max +
  // 1)
  {
    ...
    sit[location].emplace_back(constraint_table.length_min,
                               min(constraint_table.length_max + 1, MAX_TIMESTEP), false);
  } else {
    sit[location].emplace_back(0, min(constraint_table.length_max, MAX_TIMESTEP - 1) + 1, false);
  }
  // path table
  if (constraint_table.path_table_for_CT != nullptr and
      !constraint_table.path_table_for_CT->table.empty()) {
    if (location < constraint_table.map_size)  // vertex conflict
    {
      ...
      for (const auto& goal_t: constraint_table.path_table_for_CT->goals[location]){ // target conflict
        if (goal_t < window)
          insert2SIT(location, goal_t, window);
      }
    } else  // edge conflict
    {
  ...
  if (constraint_table.path_table_for_CAT != nullptr and
    ...
    if (location < constraint_table.map_size)  // vertex conflict
    {
      ...
      for (const auto& goal_t: constraint_table.path_table_for_CAT->goals[location]){
        if (goal_t < window)
          insertSoftConstraint2SIT(location, goal_t, window);
      }
    } else  // edge conflict
    {
      auto from = location / constraint_table.map_size - 1;
  ...
  if (!constraint_table.cat.empty()) {
    for (auto t = 0; t < constraint_table.cat[location].size(); t++) {
      if (constraint_table.cat[location][t]) insertSoftConstraint2SIT(location, t, t + 1);
    }
...
list<tuple<int, int, int, bool, bool>> ReservationTable::get_safe_intervals(int from, int to,
                                                                            int lower_bound,
                                                                            int upper_bound) {
...
bool ReservationTable::find_safe_interval(Interval& interval, size_t location, int t_min) {
  if (t_min >= min(constraint_table.length_max, MAX_TIMESTEP - 1) + 1) return false;
  if (sit[location].empty()) updateSIT(location);
...
int ReservationTable::get_earliest_arrival_time(int from, int to, int lower_bound,
                                                int upper_bound) const {
  for (auto t = lower_bound; t < upper_bound; t++) {
    if (!constraint_table.constrained(from, to, t)) return t;
  }
  return -1;
}

int ReservationTable::get_earliest_no_collision_arrival_time(int from, int to,
